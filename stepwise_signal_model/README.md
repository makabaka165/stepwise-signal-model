...
可选扩展（偏跟踪，第 6.5 步） -> 跨 CPI 局部跟踪与波束调度
```
...
### 5.6 第 6.5 步：跨 CPI 的局部跟踪与波束调度

当前状态说明：

- 第 6.5 步当前保留为最小可运行版本
- 入口脚本是 `steps/step_06_5_cpi_track/demo_cpi_track.m`
...
- 因此它不再作为当前主流程中的“第 6 步”，而是作为第 6.5 步保留，供后续扩展与工作量补充
...
- 第 5 步负责“单个 `CPI` 内的局部检测与三波束精测角”
- 第 6.5 步负责“不同 `CPI` 之间的信息传递与闭环”
...
4. 第 6.5 步内部继续用 `u`，和第 4 步、第 5 步的俯仰处理更一致
...
5. 在当前 `CPI` 上运行第 5 步局部量测前端：继续使用当前第 5 步这条单 `CPI` 局部链路，即 `pcCube -> 局部波束形成 -> MTD -> 中心束 CFAR -> 聚类/后筛选 -> 三波束比幅测角`，输出当前 `CPI` 的目标量测候选。
...
- 第 5 步回答的是“当前这个 `CPI` 里，在已经选定的局部波束上下文中，检测到了什么，精测角结果是多少”。
- 第 6.5 步回答的是“连续多个 `CPI` 之间，应该如何利用上一时刻结果去指导下一时刻处理，并把单 `CPI` 量测串成一条连续航迹”。
...
- 先不扩展到多目标航迹管理
- 先不引入复杂的全局数据关联
- 先把“第 5 步局部量测前端 + 第 6.5 步跨 `CPI` 预测更新闭环”跑通
