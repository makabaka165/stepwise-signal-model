...
你现在处于 goal mode。请严格按照当前项目的 Goal Mode 工作流执行，但本次采用“连续执行 + 每 task 决策门控”的模式。

非常重要的目录要求：

本次不要在项目 root 下直接创建 goal-[num]/。

本次必须先创建 Step11.4 主目录：

stepwise_signal_model/steps/step_11_4_engineering_frontend_backend_chain/

然后在该 Step11.4 目录内部创建递增编号的 goal 目录：

stepwise_signal_model/steps/step_11_4_engineering_frontend_backend_chain/goal-[num]/

不得覆盖已有 goal 目录。

也就是说，goal 管理文档必须放在 Step11.4 目录内部：
...
input.md 必须完整保存本次用户原始输入，逐字保留，不得改写、总结、删减或重新组织。
也就是说，input.md 中应保存当前这整段 /goal prompt 的完整原文。

在这些 goal 文件完成并提交前，不得新增算法代码，不得修改已有 Step11.1、Step11.2、Step11.3、Step9、Step10 或 step_11_supersolution_ml。
Task1 阶段只允许：
1. 创建 Step11.4 主目录；
2. 创建 Step11.4 内部 goal-[num]/；
3. 写入 input.md / plan.md / tasks.md / context.md / records.md；
4. 创建 task_reports/；
5. 提交 goal 初始化文件。

完成 Task1 后不要停止，除非 goal 文件创建失败、需求冲突或出现禁止继续的情况。继续按 tasks.md 顺序执行后续 task。
...
2. 每个 task 完成后必须：
   ...
   - git diff --stat；
   - git commit；
   - 根据结果选择 GO / CONTINUE_WITH_RISK / STOP_NEEDS_USER / STOP_LOGICAL_FAILURE / ROLLBACK。
3. 如果决策是 GO，自动继续下一个 task。
4. 如果决策是 CONTINUE_WITH_RISK，也继续下一个 task，但必须在 records.md 和 task report 里写明：
   - 哪个结果不符合预期；
   - 为什么不阻断后续；
   - 后续哪个 task 需要继续验证该风险。
5. 如果决策是 STOP_NEEDS_USER、STOP_LOGICAL_FAILURE 或 ROLLBACK，立即停止执行并汇报。
6. 每 3 个 task 后必须执行大型全面 review/debug 循环。
7. 大型 review 通过后可以继续自动执行；不通过则停止。
8. 每次上下文压缩后，必须全量读取：
   - input.md
   - plan.md
   - tasks.md
...
9. 全部 task 完成后执行最终最大 review：
   ...
   - 结果可复现；
   - 文档完整性。
10. 最终 review 后写出 ACCEPT_AND_END / ACCEPT_WITH_RISKS_AND_END / REJECT_AND_ROLLBACK / ACCEPT_AND_CONTINUE_ITERATION，并将 goal 标记完成或停止。
...
-> degree-based coarse-to-fine search
-> 局部双目标增强测角输出

关键背景：

1. Step11.1 已验证 controlled pair2d beamspace ML。
2. Step11.2 推荐 W = greedy_combined_B7。
3. Step11.3 推荐 degree-based coarse-to-fine search：
   - topK = 3
...
1. 前端 CFAR / 三波束 / 二维波束粗测角通常输出一个检测单元、一个主导粗峰、一个 coarse angle，不严格判断物理目标数量。
2. Step11 后端 controlled pair2d 是局部未分辨双目标增强测角。
3. 二者通过“single coarse peak 内部可能存在 unresolved pair”统一。
4. 普通单目标默认使用前端测角；疑似未分辨局部簇才调用 pair2d enhanced mode。
5. 不要宣称完整自动单双目标模型选择完成。
6. 不要宣称完整工程闭环最终完成。
7. 如果串联失败，要输出 failure_reason 和模块化诊断。

工程解释要求：

1. 前端输出 coarse center / search window / beam energy / cluster indicator。
2. 后端只把 coarse center 当作局部搜索先验。
3. 前端单峰不等于物理单目标。
4. 前端也不需要先检测出两个目标。
5. Step11.4 要验证的是接口级链路：
   - 前端粗角是否足够准确；
   - synthetic frontend backend 是否接近 oracle backend；
   - 普通单目标是否应绕开 pair2d；
   - 若失败，瓶颈在前端还是后端。
...
2. 不要修改 Step11.2。
3. 不要修改 Step11.3。
4. 不要修改 Step9、Step10、step_11_supersolution_ml。
5. 不要用真值构造 frontend coarse center。
6. 不要把前端单峰解释成物理单目标。
7. 不要强制所有单目标进入 pair2d 后端并宣称正确。
8. 不要宣称完整目标数自动判断完成。
9. 不要做 AP。
10. 不要做 full4D。
11. 不要在阵元域直接做 ML 作为主结果。
12. 不要隐藏串联失败。
13. 如果失败，要输出 failure_reason 和模块化诊断。

执行要求：

1. 先执行 Task1。
2. Task1 中必须：
   - 创建 stepwise_signal_model/steps/step_11_4_engineering_frontend_backend_chain/
   - 在该目录内创建 goal-[num]/
   - 在 goal-[num]/ 中创建 input.md、plan.md、tasks.md、context.md、records.md、task_reports/
   - 将当前这整段 /goal prompt 原文完整写入 input.md
   - 将下面给出的 plan.md 内容写入 plan.md
   - 将下面给出的 tasks.md 内容写入 tasks.md
   - 将下面给出的 context.md 内容写入 context.md
   - 将下面给出的 records.md 内容写入 records.md
3. plan.md 由当前 prompt 提供的 plan 内容写入，不要把 plan.md 内容写进 input.md 之外的地方。
4. tasks.md 由当前 prompt 提供的 tasks 内容写入。
5. context.md 用于保存跨上下文压缩需要保留的关键上下文。
6. records.md 用于保存每个 task 的预期结果、实际结果、对齐程度、成因分析和决策。
7. Task1 成功后自动继续 Task2，除非遇到停止条件。
