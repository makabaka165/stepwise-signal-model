...
log_lines = {};
log_lines = append_log(log_lines, 'Step 7.5 Route B stability recheck for bw/8~bw/10');
...
params.recheck_note = 'Step 7.5 stability recheck for bw/8~bw/10 with Metkl=500 and snr=-4:2:30.';

result_dir = fullfile(fileparts(mfilename('fullpath')), 'results_step7_5_routeB_recheck_bw8_to_bw10_metkl500');
if ~exist(result_dir, 'dir')
    mkdir(result_dir);
end

log_path = fullfile(result_dir, 'step7_5_routeB_recheck_bw8_to_bw10.log');
...
csv_path = fullfile(result_dir, 'step7_5_routeB_recheck_bw8_to_bw10_summary.csv');
...
mat_path = fullfile(result_dir, 'step7_5_routeB_recheck_bw8_to_bw10_result.mat');
save(mat_path, ...
    'params', ...
    'sep_factor_list', ...
    'snr_list', ...
...
title('Step 7.5 Route B recheck bw/8~bw/10: tol success rate');
legend(legend_labels, 'Location', 'best');
saveas(fig1, fullfile(result_dir, 'step7_5_routeB_recheck_bw8_to_bw10_tol_success.png'));
close(fig1)
...
xlabel('SNR (dB)');
ylabel('RMSE (deg)');
title('Step 7.5 Route B recheck bw/8~bw/10: RMSE');
legend(legend_labels, 'Location', 'best');
saveas(fig2, fullfile(result_dir, 'step7_5_routeB_recheck_bw8_to_bw10_rmse.png'));
close(fig2)
...
title('Step 7.5 Route B recheck bw/8~bw/10: raw success rate');
legend(legend_labels, 'Location', 'best');
saveas(fig3, fullfile(result_dir, 'step7_5_routeB_recheck_bw8_to_bw10_raw_success.png'));
close(fig3)
...
title('Step 7.5 Route B recheck bw/8~bw/10: mean number of peaks');
legend(legend_labels, 'Location', 'best');
saveas(fig4, fullfile(result_dir, 'step7_5_routeB_recheck_bw8_to_bw10_mean_num_peaks.png'));
close(fig4)
...
title('Step 7.5 Route B recheck bw/8~bw/10: SNR90');
saveas(fig5, fullfile(result_dir, 'step7_5_routeB_recheck_bw8_to_bw10_snr90.png'));
close(fig5)
