function [est, score_info, debug] = search_pair2d_degree_grid_precomputed(Z, W, x, y, z, lambda, grid_cfg, manifold_opts, search_opts, varargin)
%SEARCH_PAIR2D_DEGREE_GRID_PRECOMPUTED Controlled pair2d grid search with physical el_sep values.

if nargin < 9
    error('search_pair2d_degree_grid_precomputed:NotEnoughInputs', ...
        'Z, W, x, y, z, lambda, grid_cfg, manifold_opts, and search_opts are required.');
end
...
B = size(W, 2);
if size(Z, 1) ~= B
    error('search_pair2d_degree_grid_precomputed:ShapeMismatch', 'size(Z,1) must equal size(W,2).');
end
...
if explicit_rows_mode
    ...
    if isempty(valid_rows)
        error('search_pair2d_degree_grid_precomputed:NoValidElPair', 'No valid degree-based elevation pair exists.');
    end
end
...
if size(G_grid, 1) ~= B || size(G_grid, 2) ~= N_az || size(G_grid, 3) ~= N_el_values
    error('search_pair2d_degree_grid_precomputed:GridShapeMismatch', 'Precomputed G grid has unexpected dimensions.');
end
...
for iRow = 1:numel(valid_rows)
    ...
    if isempty(iEl1) || isempty(iEl2)
        error('search_pair2d_degree_grid_precomputed:ElLookupFailed', 'Could not locate el values in precomputed grid.');
    end
    G1 = G_use_grid(:, :, iEl1);
    G2 = G_use_grid(:, :, iEl2);
    if explicit_rows_mode
...
if ~isfinite(max_score)
    error('search_pair2d_degree_grid_precomputed:NoFiniteScore', 'No finite degree-based DML score was found.');
end
...
debug = struct();
debug.search_mode = 'degree_based_pair2d_grid';
...
if mod(numel(varargin), 2) ~= 0
    error('search_pair2d_degree_grid_precomputed:InvalidNameValue', 'Name-value options must be paired.');
end
for idx = 1:2:numel(varargin)
    ...
    switch name
        ...
        otherwise
            error('search_pair2d_degree_grid_precomputed:UnknownOption', 'Unknown option: %s', name);
    end
end
...
if ~explicit_rows_mode && (~isfield(grid_cfg, 'az_grid') || isempty(grid_cfg.az_grid))
    error('search_pair2d_degree_grid_precomputed:MissingAzGrid', 'grid_cfg.az_grid is required.');
end
if explicit_rows_mode
    candidate_rows = normalize_candidate_rows_local(grid_cfg.candidate_rows);
...
elseif isfield(grid_cfg, 'el_grid')
    ...
    error('search_pair2d_degree_grid_precomputed:MissingElGrid', 'grid_cfg.el_center_grid or grid_cfg.el_grid is required.');
end
if ~isfield(grid_cfg, 'el_sep_deg_list')
    error('search_pair2d_degree_grid_precomputed:MissingElSepDegList', 'grid_cfg.el_sep_deg_list is required.');
end
...
if ~isfield(search_opts, 'whitening_mode')
    error('search_pair2d_degree_grid_precomputed:MissingWhiteningMode', 'search_opts.whitening_mode is required.');
end
...
if ~isnumeric(candidate_rows) || size(candidate_rows, 2) < 7
    error('search_pair2d_degree_grid_precomputed:InvalidCandidateRows', ...
        'grid_cfg.candidate_rows must be a numeric matrix with columns [az1 az2 el1 el2 el_center el_sep_deg orientation].');
end
...
if isempty(candidate_rows)
    error('search_pair2d_degree_grid_precomputed:NoExplicitCandidateRows', ...
...
if any(~tf1 | ~tf2)
    error('search_pair2d_degree_grid_precomputed:AzLookupFailed', ...
