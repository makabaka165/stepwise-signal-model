function [est_adaptive, debug_adaptive] = search_pair2d_adaptive_topk_window_v2(Z, W, x, y, z, lambda, ...
    grid_cfg_coarse, base_refine_cfg, manifold_opts, search_opts, policy_cfg, top_candidates, coarse_debug)
%SEARCH_PAIR2D_ADAPTIVE_TOPK_WINDOW_V2 Stage2 calibrated adaptive wrapper.
%
% Optional top_candidates/coarse_debug inputs let Stage2 scan many explicit
% policy configs without rerunning the coarse topK search. The Step11.3 local
% refine helper and controlled pair2d beamspace ML score remain unchanged.

if nargin < 11 || isempty(policy_cfg)
    policy_cfg = struct();
end
policy_cfg = fill_policy_defaults_local(policy_cfg);
if nargin < 12 || isempty(top_candidates) || nargin < 13 || isempty(coarse_debug)
    [top_candidates, coarse_debug] = search_pair2d_coarse_grid_topk(Z, W, x, y, z, lambda, ...
        grid_cfg_coarse, manifold_opts, search_opts, policy_cfg.topK_max);
end
...
if ~isfield(policy_cfg, 'config_id')
    policy_cfg.config_id = 0;
end
if ~isfield(policy_cfg, 'config_name')
    policy_cfg.config_name = 'unnamed_v2_policy';
end
end

function debug_adaptive = make_debug_template_local(policy_cfg)
debug_adaptive = struct();
debug_adaptive.search_mode = 'stage2_calibrated_likelihood_uncertainty_adaptive_topk_window';
