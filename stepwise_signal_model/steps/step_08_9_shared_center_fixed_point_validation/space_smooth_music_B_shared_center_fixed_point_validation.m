% Step 8.9 shared-center fixed-point influence validation.
%
% Scope:
% 1) Reuse the Step 8.8 frontend and Step 8.7 shared-center lazy cascade.
% 2) Quantize Y_work and/or local steering/template/cache data, then
%    dequantize back into the same MATLAB lazy cascade.
% 3) Validate numerical finite-wordlength sensitivity only.
%
% This is not FPGA HDL, not full bit-true fixed point, not MC=100, and not
% dual-center. Step 8.7 route gates and thresholds are not changed.
...
result_dir = fullfile(script_dir, 'results_step8_9_shared_center_fixed_point_validation');
if ~exist(result_dir, 'dir')
    mkdir(result_dir);
end

log_path = fullfile(result_dir, 'step8_9_shared_center_fixed_point_validation.log');
fid_log = fopen(log_path, 'w');
cleanup_log = onCleanup(@() safe_fclose_local(fid_log));

summary_csv_path = fullfile(result_dir, 'step8_9_shared_center_fixed_point_summary.csv');
keypoints_csv_path = fullfile(result_dir, 'step8_9_shared_center_fixed_point_keypoints.csv');
trial_csv_path = fullfile(result_dir, 'step8_9_shared_center_fixed_point_trial.csv');
storage_csv_path = fullfile(result_dir, 'step8_9_shared_center_fixed_point_storage_estimate.csv');
mat_path = fullfile(result_dir, 'step8_9_shared_center_fixed_point_result.mat');
record_doc_path = fullfile(script_dir, '第8.9步_shared-center主线定点量化影响验证记录.md');

quick_mode = strcmpi(getenv('STEP89_QUICK_MODE'), '1');
if quick_mode
    Metkl = 5;
else
    Metkl = 30;
...
quant_modes = build_step89_quant_modes_local();
total_base_trials = numel(scenarios) * Metkl;
total_mode_trials = total_base_trials * numel(quant_modes);

log_msg_local(fid_log, 'Step 8.9 shared-center fixed-point influence validation');
...
log_msg_local(fid_log, 'Quant modes: %s', strjoin(cellstr([quant_modes.name]), ', '));
log_msg_local(fid_log, 'Default Y_work Doppler mode from Step 8.8B: no_derotation.');
log_msg_local(fid_log, 'Scope: same frontend_out and selected 65-column work array; only finite-wordlength representation changes.');
log_msg_local(fid_log, 'No dual-center, no FPGA HDL, no bit-true EVD, no V2, no weak-target SIC, no anti-phase derivative.');
log_msg_local(fid_log, 'Step 8.7 lazy route thresholds are copied from part 7B and not changed.');
log_msg_local(fid_log, 'Original Step 8.7/8.8/8.8B result directories are not overwritten.');
...
log_msg_local(fid_log, 'Doppler axis retained for interface fields: fdAxis uses fftshift order, default derotation_mode=none.');
...
summary_tbl = build_step89_summary_table_local(trial_tbl, steering_error_tbl, quant_modes);
storage_tbl = build_step89_storage_estimate_local(cfg88);
keypoints_tbl = build_step89_keypoints_local(trial_tbl, summary_tbl, Metkl, quick_mode);
...
plot_step89_steering_error_local(steering_error_tbl, fullfile(result_dir, 'quantization_steering_error.png'));
plot_step89_ywork_error_local(summary_tbl, fullfile(result_dir, 'quantization_ywork_error.png'));
plot_step89_success_local(summary_tbl, fullfile(result_dir, 'success_by_quant_mode.png'));
plot_step89_agreement_local(summary_tbl, 'route', fullfile(result_dir, 'route_agreement_by_quant_mode.png'));
plot_step89_agreement_local(summary_tbl, 'confidence', fullfile(result_dir, 'confidence_agreement_by_quant_mode.png'));
plot_step89_az_el_error_local(summary_tbl, fullfile(result_dir, 'az_el_error_by_quant_mode.png'));
plot_step89_false_high_boundary_local(summary_tbl, fullfile(result_dir, 'false_high_boundary_by_quant_mode.png'));
plot_step89_storage_local(storage_tbl, fullfile(result_dir, 'template_storage_by_wordlength.png'));

save(mat_path, 'trial_tbl', 'summary_tbl', 'keypoints_tbl', 'cfg88', 'array_geom', ...
    'pc_model', 'elapsed_sec', 'Metkl', 'quick_mode', 'quant_modes', ...
    'storage_tbl', 'steering_error_tbl', '-v7.3');

write_step89_record_doc_local(record_doc_path, keypoints_tbl, summary_tbl, storage_tbl, ...
    steering_error_tbl, result_dir, elapsed_sec, Metkl, quick_mode, cfg88, quant_modes);
...
function quant_modes = build_step89_quant_modes_local()
    quant_modes = repmat(struct('name', "", 'coeff_kind', "double", 'coeff_bits', NaN, ...
        'y_kind', "double", 'y_bits', NaN, 'note', ""), 1, 7);
    quant_modes(1) = make_quant_mode_local("double_baseline", "double", NaN, "double", NaN, ...
        "all-double Step 8.7/8.8 shared-center baseline");
    quant_modes(2) = make_quant_mode_local("float32_all", "float32", NaN, "float32", NaN, ...
        "Y_work and complex steering/cache cast to single then back to double");
    quant_modes(3) = make_quant_mode_local("coeff_int16_only", "fixed", 16, "double", NaN, ...
        "complex steering/template/cache int16, Y_work double");
    quant_modes(4) = make_quant_mode_local("ywork_int16_only", "double", NaN, "fixed", 16, ...
        "Y_work int16 block floating, coefficients double");
    quant_modes(5) = make_quant_mode_local("combined_int16", "fixed", 16, "fixed", 16, ...
        "Y_work and complex steering/template/cache int16");
    quant_modes(6) = make_quant_mode_local("combined_int18", "fixed", 18, "fixed", 18, ...
        "Y_work and complex steering/template/cache int18");
    quant_modes(7) = make_quant_mode_local("combined_int24", "fixed", 24, "fixed", 24, ...
        "Y_work and complex steering/template/cache int24");
end

function qmode = make_quant_mode_local(name, coeff_kind, coeff_bits, y_kind, y_bits, note)
    qmode = struct('name', string(name), 'coeff_kind', string(coeff_kind), 'coeff_bits', coeff_bits, ...
        'y_kind', string(y_kind), 'y_bits', y_bits, 'note', string(note));
end
...
function diag = compute_steering_error_diag_local(ctx_ref, ctx_q, qmode, cfg88, center_col)
    ...
    if qmode.coeff_kind == "fixed"
        [Wr, ~] = quantize_real_unit_local(real(W_ref), qmode.coeff_bits);
        [Wi, ~] = quantize_real_unit_local(imag(W_ref), qmode.coeff_bits);
        W_q = complex(Wr, Wi);
