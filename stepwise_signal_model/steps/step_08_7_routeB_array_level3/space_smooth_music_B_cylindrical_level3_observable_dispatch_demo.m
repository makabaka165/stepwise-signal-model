...
log_path = fullfile(result_dir, 'step8_7_6_observable_dispatch_demo.log');
...
keypoints_csv_path = fullfile(result_dir, 'step8_7_6_observable_dispatch_demo_keypoints.csv');
mat_path = fullfile(result_dir, 'step8_7_6_observable_dispatch_demo_result.mat');
record_doc_path = fullfile(script_dir, '第8.7步_6_观测量驱动工程分流验证记录.md');

log_msg(fid_log, 'Step 08.7 part 6: observable-dispatch engineering demo');
log_msg(fid_log, 'Scope: compare oracle-label dispatch with observable dispatch. Observable dispatch does not use beta/phase/rho/el_true/el_diff as inputs. No fullscan, no V2, no complex-gain q, no 4D search, no edits to step 8.6 or prior step 8.7 outputs.');
...
write_record_doc_v2_local(record_doc_path, summary_tbl, keypoints_tbl, result_dir, elapsed_sec, Metkl);
...
function result = apply_observable_dispatch_rule_local(route_map, min_sep, max_sep)
    ...
    if music2d_ok && pair_ok
        ...
        if getfield_default_local(pair2d, 'pair_local_confidence_proxy', 0) >= 0.10
    ...
    elseif music_ok
        ...
        if getfield_default_local(music, 'peak_prominence_ratio', 0) >= 0.70
            result.confidence_flag = "high";
        else
            result.confidence_flag = "medium";
        end
    ...
    elseif refocus_ok
        ...
        if getfield_default_local(refocus, 'focused_power_peak_sharpness', 0) >= 0.07
            result.confidence_flag = "high";
        else
            result.confidence_flag = "medium";
        end
    ...
    result.low_confidence_flag = strcmp(result.confidence_flag, "low") || strcmp(result.recommended_route, "low_confidence");
end
...
function ok = is_rank1_route_reliable_local(result, min_sep, max_sep)
    ...
    ok = all(isfinite(result.az_est)) && all(isfinite(result.el_est)) && ...
        ...
        isfinite(residual) && residual < 1e-3 && isfinite(gap) && gap >= 1e-3;
end
...
function store = store_dispatch_compare_trial_local(store, imc, oracle_result, observable_result, theta_true, el_true, az_tol, el_tol)
    ...
    observable_normal_high = strcmp(conf, "high") && ~strcmp(observable_route, "low_confidence");
    ...
    store.low_confidence(imc) = strcmp(conf, "low") || strcmp(observable_route, "low_confidence");
...
function row = make_summary_row_local(sc, route_name, metrics, compare_metrics)
    row = {sc.stage, sc.group, sc.case_name, sc.el_b, sc.el_diff, sc.el_mismatch, ...
        ...
        compare_metrics.low_confidence_rate, compare_metrics.false_high_confidence_rate, ...
...
function names = summary_var_names_local()
    names = {'stage', 'group', 'case_name', 'el_b_deg', 'el_diff_deg', 'el_mismatch_deg', ...
        ...
        'observable_vs_oracle_success_gap', 'dispatch_low_confidence_rate', ...
