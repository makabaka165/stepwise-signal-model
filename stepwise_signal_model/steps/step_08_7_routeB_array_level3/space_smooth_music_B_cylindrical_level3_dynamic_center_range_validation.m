% Step 8.7 Part 9: dynamic center column selection and Delta-theta range validation.
% This file reuses the part-7B lazy cascade local functions below. The copied
% part-7B top-level driver is intentionally skipped after this part-9 driver.
...
result_dir = fullfile(script_dir, 'results_step8_7_9_dynamic_center_range_validation');
if ~exist(result_dir, 'dir')
    mkdir(result_dir);
end

log_path = fullfile(result_dir, 'step8_7_9_dynamic_center_range_validation.log');
fid_log = fopen(log_path, 'w');
cleanup_log = onCleanup(@() safe_fclose_local(fid_log));

coverage_csv_path = fullfile(result_dir, 'step8_7_9_dynamic_center_range_coverage.csv');
storage_csv_path = fullfile(result_dir, 'step8_7_9_template_storage_compute_estimate.csv');
doa_summary_csv_path = fullfile(result_dir, 'step8_7_9_dynamic_center_range_doa_summary.csv');
keypoints_csv_path = fullfile(result_dir, 'step8_7_9_dynamic_center_range_keypoints.csv');
mat_path = fullfile(result_dir, 'step8_7_9_dynamic_center_range_result.mat');
record_doc_path = fullfile(script_dir, '第8.7步_9_双目标动态选列与Delta搜索范围验证记录.md');

log_msg(fid_log, 'Step 08.7 part 9: dynamic center column selection and Delta-theta range validation.');
log_msg(fid_log, 'Scope: no FPGA, no fixed point, no MC=100, no new DOA algorithm, no threshold or dispatch-rule edits.');
log_msg(fid_log, 'Part C reuses the part-7B lazy cascade functions and changes only the local azimuth grid range R.');

R_list = [0.6, 1.0, 1.5, 2.0];
delta_step_deg = 0.02;
beta_list_analytic = [1, 0.8, 0.5, 0.3];
snr_list = [8, 16];
Metkl = 20;
quick_mode = false;
if strcmpi(getenv('STEP87_9_QUICK_MODE'), '1') || strcmpi(getenv('STEP87_QUICK_MODE'), '1')
    Metkl = 5;
    quick_mode = true;
end
T_snap = 260;
...
[column_spacing_deg, half_column_spacing_deg, phi_col_from_xy] = column_spacing_from_actual_array_local(cfg);
delta_list = [-half_column_spacing_deg, -0.5 * half_column_spacing_deg, 0, ...
    0.5 * half_column_spacing_deg, half_column_spacing_deg];
delta_list_doa = [-half_column_spacing_deg, 0, half_column_spacing_deg];

Q = cfg.beam.subNaz;
d_eq = 2*pi * cfg.arr.R / cfg.arr.Naz;
bw_eq_65 = 50.8 * 1.45 * cfg.arr.lambda / ((Q - 1) * d_eq);
theta_sep_ref = bw_eq_65 / 10;
pair_sep_list_coverage = [theta_sep_ref, 0.5, 0.8, 1.2];
pair_sep_list_doa = [theta_sep_ref, 0.5, 0.8];
pair_sep_list_large_el = [theta_sep_ref, 0.5];

log_msg(fid_log, 'Column spacing from actual XY coordinates: %.9f deg, half spacing: %.9f deg.', ...
    column_spacing_deg, half_column_spacing_deg);
log_msg(fid_log, 'sep_factor=10 equivalent theta_sep=%.9f deg, replacing nominal 0.264 deg.', theta_sep_ref);
log_msg(fid_log, 'Run control: Metkl=%d, quick_mode=%d, R=%s, SNR=%s.', ...
    Metkl, quick_mode, mat2str(R_list), mat2str(snr_list));

coverage_tbl = build_dynamic_center_coverage_table_local( ...
    R_list, pair_sep_list_coverage, delta_list, beta_list_analytic, column_spacing_deg);
coverage_summary_tbl = summarize_dynamic_coverage_local(coverage_tbl);
storage_tbl = build_template_storage_compute_table_local(R_list, delta_step_deg, el_grid);

log_msg(fid_log, 'Part A coverage rows=%d. Part B storage rows=%d.', height(coverage_tbl), height(storage_tbl));
log_msg(fid_log, 'Part C DOA route validation started.');
tic_doa = tic;
[doa_trial_tbl, doa_summary_tbl] = run_dynamic_center_doa_validation_local( ...
    cfg, R_list, delta_step_deg, snr_list, Metkl, T_snap, Lc, theta_c_actual, ...
    half_column_spacing_deg, pair_sep_list_doa, pair_sep_list_large_el, delta_list_doa, ...
    K_phi_level2, K_phi_music, K_z_music, K_phi_covfit, K_z_covfit, ...
    el_grid, el_refocus_grid, el_bank, az_tol_deg, el_tol_deg, ...
    min_pair_sep_deg, max_pair_sep_deg, base_seed, fid_log);
elapsed_doa_sec = toc(tic_doa);
log_msg(fid_log, 'Part C DOA route validation finished in %.2f sec, trial rows=%d.', ...
    elapsed_doa_sec, height(doa_trial_tbl));

keypoints_tbl = build_dynamic_range_keypoints_local( ...
    coverage_summary_tbl, storage_tbl, doa_summary_tbl, column_spacing_deg, half_column_spacing_deg);

log_msg(fid_log, 'Writing CSV/MAT outputs.');
writetable(coverage_tbl, coverage_csv_path);
writetable(storage_tbl, storage_csv_path);
writetable(doa_summary_tbl, doa_summary_csv_path);
writetable(keypoints_tbl, keypoints_csv_path);

log_msg(fid_log, 'Rendering plots.');
plot_coverage_vs_R_by_center_mode_local(coverage_tbl, fullfile(result_dir, 'coverage_vs_R_by_center_mode.png'));
plot_required_R_vs_pair_sep_local(coverage_tbl, fullfile(result_dir, 'required_R_vs_pair_sep.png'));
plot_delta_theta_distribution_by_mode_local(coverage_tbl, fullfile(result_dir, 'delta_theta_distribution_by_mode.png'));
plot_template_size_vs_R_local(storage_tbl, fullfile(result_dir, 'template_size_vs_R.png'));
plot_compute_ratio_vs_R_local(storage_tbl, fullfile(result_dir, 'compute_ratio_vs_R.png'));
plot_doa_success_vs_R_local(doa_summary_tbl, fullfile(result_dir, 'doa_success_vs_R.png'));
plot_doa_low_confidence_vs_R_local(doa_summary_tbl, fullfile(result_dir, 'low_confidence_vs_R.png'));
plot_doa_runtime_vs_R_local(doa_summary_tbl, fullfile(result_dir, 'runtime_vs_R.png'));

save(mat_path, 'coverage_tbl', 'coverage_summary_tbl', 'storage_tbl', 'doa_trial_tbl', ...
    'doa_summary_tbl', 'keypoints_tbl', 'R_list', 'delta_step_deg', 'el_grid', ...
    'column_spacing_deg', 'half_column_spacing_deg', 'phi_col_from_xy', 'theta_sep_ref', ...
    'pair_sep_list_coverage', 'pair_sep_list_doa', 'Metkl', 'quick_mode', 'elapsed_doa_sec', 'cfg', '-v7');

write_dynamic_center_range_record_doc_local(record_doc_path, result_dir, keypoints_tbl, ...
    coverage_summary_tbl, storage_tbl, doa_summary_tbl, column_spacing_deg, half_column_spacing_deg, ...
    theta_sep_ref, Metkl, quick_mode, elapsed_doa_sec);

log_msg(fid_log, 'Outputs written to %s', result_dir);
log_msg(fid_log, 'Record written to %s', record_doc_path);

return

% ---- Legacy part-7B copied driver below. It is intentionally skipped above;
% local functions after the legacy driver are reused unchanged by part 9. ----
...
log_msg(fid_log, 'Outputs written to %s', result_dir);
function [column_spacing_deg, half_column_spacing_deg, phi_col_deg] = column_spacing_from_actual_array_local(cfg)
    arrInfo = arr_cyl(cfg, 0);
    phi_col_deg = sort(mod(atan2d(arrInfo.Y(:, 1), arrInfo.X(:, 1)), 360));
    dphi = diff([phi_col_deg(:); phi_col_deg(1) + 360]);
    column_spacing_deg = median(dphi);
    half_column_spacing_deg = column_spacing_deg / 2;
end

function tbl = build_dynamic_center_coverage_table_local(R_list, pair_sep_list, delta_list, beta_list, column_spacing_deg)
    rows = {};
    for isep = 1:numel(pair_sep_list)
        pair_sep = pair_sep_list(isep);
        for id = 1:numel(delta_list)
            delta = delta_list(id);
            d_pair = [delta - pair_sep / 2, delta + pair_sep / 2];
            rows = add_coverage_rows_for_R_local(rows, "pair_center", NaN, "", pair_sep, delta, R_list, d_pair);

            d_strong_right = [delta, delta + pair_sep];
            d_strong_left = [delta, delta - pair_sep];
            rows = add_coverage_rows_for_R_local(rows, "strong_target", NaN, "right", pair_sep, delta, R_list, d_strong_right);
            rows = add_coverage_rows_for_R_local(rows, "strong_target", NaN, "left", pair_sep, delta, R_list, d_strong_left);

            for ib = 1:numel(beta_list)
                beta = beta_list(ib);
                theta1 = delta - pair_sep / 2;
                theta2 = delta + pair_sep / 2;
                theta_eff = (theta1 + beta * theta2) / (1 + beta);
                theta_c = nearest_column_from_spacing_local(theta_eff, column_spacing_deg);
                d_obs = [theta1 - theta_c, theta2 - theta_c];
                rows = add_coverage_rows_for_R_local(rows, "observed_coarse_peak_analytic", beta, "", ...
                    pair_sep, delta, R_list, d_obs);
            end
        end
    end
    tbl = cell2table(rows, 'VariableNames', dynamic_coverage_var_names_local());
end

function rows = add_coverage_rows_for_R_local(rows, center_mode, beta, weak_side, pair_sep, delta, R_list, dtheta)
    for ir = 1:numel(R_list)
        R = R_list(ir);
        target1_inside = abs(dtheta(1)) <= R + 1e-12;
        target2_inside = abs(dtheta(2)) <= R + 1e-12;
        required_R = max(abs(dtheta));
        rows(end+1, :) = { ...
            string(center_mode), beta, string(weak_side), pair_sep, delta, R, ...
            dtheta(1), dtheta(2), target1_inside, target2_inside, ...
            target1_inside && target2_inside, required_R, required_R, R - required_R}; %#ok<AGROW>
    end
end

function names = dynamic_coverage_var_names_local()
    names = {'center_mode', 'beta', 'weak_side', 'pair_sep', 'delta', 'R', ...
        'delta_theta1', 'delta_theta2', 'target1_inside_range', 'target2_inside_range', ...
        'both_inside_range', 'max_abs_delta_target', 'required_R', 'margin'};
end

function theta_c = nearest_column_from_spacing_local(theta, column_spacing_deg)
    q0 = floor(theta / column_spacing_deg);
    candidates = column_spacing_deg * ((q0 - 1):(q0 + 2));
    [~, idx] = min(abs(theta - candidates));
    theta_c = candidates(idx);
end

function summary_tbl = summarize_dynamic_coverage_local(tbl)
    modes = unique(string(tbl.center_mode), 'stable');
    rows = {};
    for im = 1:numel(modes)
        mode = modes(im);
        mode_mask = string(tbl.center_mode) == mode;
        worst_required = max(tbl.required_R(mode_mask));
        r_values = unique(tbl.R(mode_mask), 'stable');
        rec_R = NaN;
        for ir = 1:numel(r_values)
            if mean(tbl.both_inside_range(mode_mask & tbl.R == r_values(ir))) >= 1
                rec_R = r_values(ir);
                break
            end
        end
        for ir = 1:numel(r_values)
            mask = mode_mask & tbl.R == r_values(ir);
            rows(end+1, :) = {mode, r_values(ir), ...
                mean(tbl.target1_inside_range(mask)), mean(tbl.target2_inside_range(mask)), ...
                mean(tbl.both_inside_range(mask)), max(tbl.required_R(mask)), worst_required, rec_R}; %#ok<AGROW>
        end
    end
    summary_tbl = cell2table(rows, 'VariableNames', ...
        {'center_mode', 'R', 'coverage_rate_target1', 'coverage_rate_target2', ...
        'coverage_rate_both', 'worst_required_R_at_R', 'worst_required_R', 'recommended_min_R_for_mode'});
end

function tbl = build_template_storage_compute_table_local(R_list, delta_step_deg, el_grid)
    rows = {};
    N_el = numel(el_grid);
    N_delta_ref = floor(2 * R_list(1) / delta_step_deg + 1e-9) + 1;
    N_grid_ref = N_delta_ref * N_el;
    template_defs = { ...
        "2d_music_center_subarray", 160, true; ...
        "level2_combined_steering", 65, true; ...
        "refocus_elevation_weight", 32, false};
    for ir = 1:numel(R_list)
        R = R_list(ir);
        N_delta = floor(2 * R / delta_step_deg + 1e-9) + 1;
        N_grid = N_delta * N_el;
        compute_ratio = N_grid / N_grid_ref;
        for it = 1:size(template_defs, 1)
            template_type = template_defs{it, 1};
            K = template_defs{it, 2};
            uses_delta = template_defs{it, 3};
            if uses_delta
                num_complex = N_delta * N_el * K;
                N_delta_storage = N_delta;
            else
                num_complex = N_el * K;
                N_delta_storage = 1;
            end
            rows(end+1, :) = {template_type, R, N_delta, N_delta_storage, N_el, N_grid, ...
                compute_ratio, K, num_complex, ...
                num_complex * 8 / 2^20, num_complex * 4 / 2^20, ...
                num_complex * 6 / 2^20, num_complex * 6 / 2^20}; %#ok<AGROW>
        end
    end
    tbl = cell2table(rows, 'VariableNames', ...
        {'template_type', 'R', 'N_delta', 'N_delta_storage', 'N_el', 'N_grid', ...
        'compute_ratio_vs_R06', 'K', 'num_complex', ...
        'template_size_MB_float32', 'template_size_MB_int16', ...
        'template_size_MB_int18', 'template_size_MB_int24'});
end

function [trial_tbl, summary_tbl] = run_dynamic_center_doa_validation_local( ...
    cfg, R_list, delta_step_deg, snr_list, Metkl, T_snap, Lc, theta_c_actual, ...
    half_column_spacing_deg, pair_sep_list_doa, pair_sep_list_large_el, delta_list_doa, ...
    K_phi_level2, K_phi_music, K_z_music, K_phi_covfit, K_z_covfit, ...
    el_grid, el_refocus_grid, el_bank, az_tol_deg, el_tol_deg, ...
    ...
    scenarios = build_dynamic_doa_scenarios_local(theta_c_actual, half_column_spacing_deg, ...
        pair_sep_list_doa, pair_sep_list_large_el, delta_list_doa, snr_list);
    arrInfo = arr_cyl(cfg, theta_c_actual);
    Q = cfg.beam.subNaz;
    Nel = cfg.arr.Nel;
    X3d = arrInfo.XAct(1:Q, :);
    Y3d = arrInfo.YAct(1:Q, :);
    Z3d = arrInfo.ZAct(1:Q, :);
    A_ref_2d = exp(-1j * 2*pi / cfg.arr.lambda * ...
        (X3d * cosd(theta_c_actual) + Y3d * sind(theta_c_actual)));
    [p_sel, r_sel] = make_selected_2d_subarray_positions_local(Q, Nel, K_phi_covfit, K_z_covfit);

    s1_base = exp(1j * 2*pi * (0:T_snap-1) / 17);
    v_base = exp(1j * 2*pi * (0:T_snap-1) / 23 + 1j*pi/7);

    num_rows = numel(R_list) * numel(scenarios) * Metkl;
    rows = cell(num_rows, numel(dynamic_doa_trial_var_names_local()));
    row_idx = 0;

    for ir = 1:numel(R_list)
        R = R_list(ir);
        az_grid = theta_c_actual - R:delta_step_deg:theta_c_actual + R;
        log_msg(fid_log, 'Part C precompute R=%.1f deg, N_delta=%d.', R, numel(az_grid));
        level2_pair_candidates = make_pair_candidates_local(az_grid, min_pair_sep_deg, max_pair_sep_deg);
        level2_cache_map = precompute_level2_bank_local( ...
            X3d, Y3d, Z3d, A_ref_2d, cfg.arr.lambda, az_grid, el_bank, K_phi_level2, level2_pair_candidates);
        level2_refocus_cache_map = precompute_level2_bank_local( ...
        ...
        if ~isempty(scenarios)
            warm = scenarios(1);
            sc_warm = make_dispatch_scenario_local(warm.scene, warm.case_name, warm.snr_db, ...
                warm.el_true, warm.beta, warm.phi_deg, warm.rho, warm.el_assumed);
            rng(base_seed - 10 + ir);
            s1 = s1_base .* exp(1j * 2*pi * rand);
            v = v_base .* exp(1j * 2*pi * rand);
            y_clean = make_clean_cylindrical_observations_general_local( ...
                X3d, Y3d, Z3d, A_ref_2d, cfg.arr.lambda, warm.theta1, warm.theta2, ...
                warm.el_true(1), warm.el_true(2), warm.beta, warm.phi_deg, warm.rho, s1, v);
            y_noisy = add_noise_local(y_clean, warm.snr_db);
            run_lazy_cascade_wallclock_local(y_noisy, sc_warm, X3d, Y3d, Z3d, A_ref_2d, cfg.arr.lambda, ...
                az_grid, el_refocus_grid, K_phi_level2, K_phi_music, K_z_music, K_phi_covfit, K_z_covfit, ...
                Lc, level2_cache_map, level2_refocus_cache_map, music_cache, p_sel, r_sel, ...
                [warm.theta1, warm.theta2], min_pair_sep_deg, max_pair_sep_deg);
        end

        for isc = 1:numel(scenarios)
            dyn = scenarios(isc);
            sc = make_dispatch_scenario_local(dyn.scene, dyn.case_name, dyn.snr_db, ...
                dyn.el_true, dyn.beta, dyn.phi_deg, dyn.rho, dyn.el_assumed);
            target1_inside = abs(dyn.delta_theta1) <= R + 1e-12;
            target2_inside = abs(dyn.delta_theta2) <= R + 1e-12;
            both_inside = target1_inside && target2_inside;
            log_msg(fid_log, ['Part C R=%.1f scenario=%03d/%03d scene=%s sep=%.6f delta=%.6f SNR=%g ' ...
                'inside=[%d,%d].'], R, isc, numel(scenarios), dyn.scene, dyn.pair_sep, dyn.delta, ...
                dyn.snr_db, target1_inside, target2_inside);
            for imc = 1:Metkl
                rng(base_seed + ir * 100000 + isc * 1000 + imc);
                s1 = s1_base .* exp(1j * 2*pi * rand);
                v = v_base .* exp(1j * 2*pi * rand);
                y_clean = make_clean_cylindrical_observations_general_local( ...
                    X3d, Y3d, Z3d, A_ref_2d, cfg.arr.lambda, dyn.theta1, dyn.theta2, ...
                    dyn.el_true(1), dyn.el_true(2), dyn.beta, dyn.phi_deg, dyn.rho, s1, v);
                y_noisy = add_noise_local(y_clean, dyn.snr_db);

                [result, timing] = run_lazy_cascade_wallclock_local(y_noisy, sc, X3d, Y3d, Z3d, A_ref_2d, cfg.arr.lambda, ...
                    az_grid, el_refocus_grid, K_phi_level2, K_phi_music, K_z_music, K_phi_covfit, K_z_covfit, ...
                    Lc, level2_cache_map, level2_refocus_cache_map, music_cache, p_sel, r_sel, ...
                    [dyn.theta1, dyn.theta2], min_pair_sep_deg, max_pair_sep_deg);

                success = joint_success_from_result_local(result, [dyn.theta1, dyn.theta2], dyn.el_true, az_tol_deg, el_tol_deg);
                route = string(getfield_default_local(result, 'recommended_route', ""));
                conf = string(getfield_default_local(result, 'confidence_flag', ""));
                reason = string(getfield_default_local(result, 'failure_reason', ""));
                low_conf = strcmp(conf, "low") || any(strcmp(route, ["low_confidence", "boundary_unreliable"]));
                boundary_unreliable = strcmp(route, "boundary_unreliable");
                false_high = strcmp(conf, "high") && ~success;
                oracle_boundary = sc.weak_target_flag || sc.anti_phase_flag;
                boundary_missed = oracle_boundary && strcmp(conf, "high") && ...
                    ~any(strcmp(route, ["low_confidence", "boundary_unreliable"])) && ~success;
                [az_err, ~] = pair_errors_local(result.az_est, result.el_est, [dyn.theta1, dyn.theta2], dyn.el_true);
                az_abs_mean = mean(abs(az_err), 'omitnan');
                az_abs_max = max(abs(az_err), [], 'omitnan');

                row_idx = row_idx + 1;
                rows(row_idx, :) = { ...
                    string(dyn.scene), string(dyn.case_name), R, dyn.pair_sep, string(dyn.center_mode), ...
                    dyn.delta, dyn.snr_db, imc, dyn.beta, dyn.phi_deg, dyn.rho, ...
                    dyn.el_true(1), dyn.el_true(2), dyn.theta1, dyn.theta2, dyn.theta_coarse, dyn.theta_c, ...
                    dyn.delta_theta1, dyn.delta_theta2, target1_inside, target2_inside, both_inside, ...
                    route, conf, success, false_high, boundary_missed, low_conf, boundary_unreliable, ...
                    az_abs_mean, az_abs_max, timing.t_total, double(timing.executed_2dmusic), ...
                    double(timing.executed_pair_local), timing.early_stop_stage, reason};
            end
        end
    end

    rows = rows(1:row_idx, :);
    trial_tbl = cell2table(rows, 'VariableNames', dynamic_doa_trial_var_names_local());
    summary_tbl = build_dynamic_doa_summary_table_local(trial_tbl);
end

function scenarios = build_dynamic_doa_scenarios_local(theta_c, half_spacing, pair_sep_list, pair_sep_list_large, delta_list, snr_list)
    template = struct('scene', "", 'case_name', "", 'center_mode', "", 'pair_sep', NaN, 'delta', NaN, ...
        'snr_db', NaN, 'beta', NaN, 'phi_deg', NaN, 'rho', NaN, 'el_true', [NaN, NaN], ...
        'el_assumed', NaN, 'theta1', NaN, 'theta2', NaN, 'theta_coarse', NaN, 'theta_c', NaN, ...
        'delta_theta1', NaN, 'delta_theta2', NaN);
    ...
    for isep = 1:numel(pair_sep_list)
        for id = 1:numel(delta_list)
            for isnr = 1:numel(snr_list)
                idx = idx + 1;
                scenarios(idx) = make_dynamic_doa_scene_local("equal_pair_center", "pair_center", ...
                    theta_c, pair_sep_list(isep), delta_list(id), snr_list(isnr), 1, 0, 1, [0, 0]);
            end
        end
    end
    ...
    for isep = 1:numel(pair_sep_list)
        for id = 1:numel(delta_list)
            for isnr = 1:numel(snr_list)
                idx = idx + 1;
                scenarios(idx) = make_dynamic_doa_scene_local("medium_beta_strong_center", "strong_target", ...
                    theta_c, pair_sep_list(isep), delta_list(id), snr_list(isnr), 0.5, 0, 1, [0, 0]);
            end
        end
    end
    ...
    for isep = 1:numel(pair_sep_list)
        for id = 1:numel(delta_list)
            for isnr = 1:numel(snr_list)
                idx = idx + 1;
                scenarios(idx) = make_dynamic_doa_scene_local("weak_beta_boundary", "strong_target", ...
                    theta_c, pair_sep_list(isep), delta_list(id), snr_list(isnr), 0.3, 0, 1, [0, 0]);
            end
        end
    end

    for isep = 1:numel(pair_sep_list_large)
        for isnr = 1:numel(snr_list)
            idx = idx + 1;
            scenarios(idx) = make_dynamic_doa_scene_local("large_el_reference", "pair_center", ...
                theta_c, pair_sep_list_large(isep), 0, snr_list(isnr), 1, 0, 1, [0, 5]);
        end
    end

    unused_half_spacing = half_spacing; %#ok<NASGU>
end

function sc = make_dynamic_doa_scene_local(scene, center_mode, theta_c, pair_sep, delta, snr_db, beta, phi_deg, rho, el_true)
    if center_mode == "pair_center"
        theta1 = theta_c + delta - pair_sep / 2;
        theta2 = theta_c + delta + pair_sep / 2;
        theta_coarse = theta_c + delta;
    else
        theta1 = theta_c + delta;
        theta2 = theta_c + delta + pair_sep;
        theta_coarse = theta1;
    end
    sc = struct();
    sc.scene = string(scene);
    sc.case_name = sprintf('%s_%s_sep%.6f_delta%.6f_snr%g', scene, center_mode, pair_sep, delta, snr_db);
    sc.center_mode = string(center_mode);
    ...
    sc.theta1 = theta1;
    sc.theta2 = theta2;
    sc.theta_coarse = theta_coarse;
    sc.theta_c = theta_c;
    sc.delta_theta1 = theta1 - theta_c;
    sc.delta_theta2 = theta2 - theta_c;
end

function names = dynamic_doa_trial_var_names_local()
    names = {'scene', 'case_name', 'R', 'pair_sep', 'center_mode', 'delta', 'SNR', 'mc', ...
        'beta', 'phase_deg', 'rho', 'el1', 'el2', 'theta1', 'theta2', 'theta_coarse', 'theta_c', ...
        'delta_theta1', 'delta_theta2', 'target1_inside_R', 'target2_inside_R', 'both_inside_R', ...
        'lazy_route', 'confidence_flag', 'success', 'false_high_confidence', 'boundary_missed', ...
        'low_confidence', 'boundary_unreliable', 'mean_abs_az_error', 'max_abs_az_error', ...
        'runtime_sec', 'executed_2dmusic', 'executed_pair_local', 'early_stop_stage', 'failure_reason'};
end

function summary_tbl = build_dynamic_doa_summary_table_local(trial_tbl)
    rows = {};
    rows{end+1, 1} = make_dynamic_doa_summary_row_local(trial_tbl, true(height(trial_tbl), 1), ...
        "overall", "all", NaN, NaN, "all", NaN);
    R_vals = unique(trial_tbl.R, 'stable');
    for ir = 1:numel(R_vals)
        mask = trial_tbl.R == R_vals(ir);
        rows{end+1, 1} = make_dynamic_doa_summary_row_local(trial_tbl, mask, ...
            "R_overall", "all", R_vals(ir), NaN, "all", NaN); %#ok<AGROW>
    end
    scenes = unique(string(trial_tbl.scene), 'stable');
    for is = 1:numel(scenes)
        for ir = 1:numel(R_vals)
            mask = string(trial_tbl.scene) == scenes(is) & trial_tbl.R == R_vals(ir);
            rows{end+1, 1} = make_dynamic_doa_summary_row_local(trial_tbl, mask, ...
                "scene_R", scenes(is), R_vals(ir), NaN, "all", NaN); %#ok<AGROW>
        end
    end
    for is = 1:numel(scenes)
        scene_mask = string(trial_tbl.scene) == scenes(is);
        sep_vals = unique(trial_tbl.pair_sep(scene_mask), 'stable');
        center_vals = unique(string(trial_tbl.center_mode(scene_mask)), 'stable');
        snr_vals = unique(trial_tbl.SNR(scene_mask), 'stable');
        for ir = 1:numel(R_vals)
            for ip = 1:numel(sep_vals)
                for ic = 1:numel(center_vals)
                    for isnr = 1:numel(snr_vals)
                        mask = scene_mask & trial_tbl.R == R_vals(ir) & ...
                            abs(trial_tbl.pair_sep - sep_vals(ip)) < 1e-9 & ...
                            string(trial_tbl.center_mode) == center_vals(ic) & trial_tbl.SNR == snr_vals(isnr);
                        if any(mask)
                            rows{end+1, 1} = make_dynamic_doa_summary_row_local(trial_tbl, mask, ...
                                "scene_R_sep_center_snr", scenes(is), R_vals(ir), ...
                                sep_vals(ip), center_vals(ic), snr_vals(isnr)); %#ok<AGROW>
                        end
                    end
                end
            end
    ...
    summary_tbl = cell2table(vertcat(rows{:}), 'VariableNames', dynamic_doa_summary_var_names_local());
end

function row = make_dynamic_doa_summary_row_local(tbl, mask, level, scene, R, pair_sep, center_mode, snr_val)
    if ~any(mask)
        row = cell(1, numel(dynamic_doa_summary_var_names_local()));
        return
    end
    inside_mask = mask & tbl.both_inside_R;
    outside_mask = mask & ~tbl.both_inside_R;
    if any(inside_mask)
        success_inside = mean(tbl.success(inside_mask), 'omitnan');
    else
        success_inside = NaN;
    end
    if any(outside_mask)
        failure_outside = mean(~tbl.success(outside_mask), 'omitnan');
    else
        failure_outside = NaN;
    end
    row = {string(level), string(scene), R, pair_sep, string(center_mode), snr_val, sum(mask), ...
        mean(tbl.success(mask), 'omitnan'), mean(tbl.false_high_confidence(mask), 'omitnan'), ...
        mean(tbl.boundary_missed(mask), 'omitnan'), mean(tbl.low_confidence(mask), 'omitnan'), ...
        mean(tbl.boundary_unreliable(mask), 'omitnan'), route_distribution_dynamic_local(string(tbl.lazy_route(mask))), ...
        mean(tbl.mean_abs_az_error(mask), 'omitnan'), max(tbl.max_abs_az_error(mask), [], 'omitnan'), ...
        mean(tbl.both_inside_R(mask), 'omitnan'), success_inside, failure_outside, ...
        mean(tbl.runtime_sec(mask), 'omitnan'), percentile_no_toolbox_local(tbl.runtime_sec(mask), 90), ...
        mean(tbl.executed_2dmusic(mask), 'omitnan'), mean(tbl.executed_pair_local(mask), 'omitnan')};
end

function names = dynamic_doa_summary_var_names_local()
    names = {'summary_level', 'scene', 'R', 'pair_sep', 'center_mode', 'SNR', 'num_trials', ...
        'success_rate', 'false_high_confidence_rate', 'boundary_missed_rate', ...
        'low_confidence_rate', 'boundary_unreliable_rate', 'route_distribution', ...
        'mean_abs_az_error', 'max_abs_az_error', 'both_inside_R_rate', ...
        'success_when_both_inside', 'failure_when_outside', 'mean_runtime_sec', ...
        'p90_runtime_sec', 'execution_rate_2dmusic', 'execution_rate_pair_local'};
end

function s = route_distribution_dynamic_local(routes)
    routes = routes(routes ~= "");
    if isempty(routes)
        s = "";
        return
    end
    vals = unique(routes, 'stable');
    parts = strings(1, numel(vals));
    for i = 1:numel(vals)
        parts(i) = sprintf('%s:%.3f', vals(i), mean(routes == vals(i)));
    end
    s = strjoin(parts, ';');
end

function keypoints = build_dynamic_range_keypoints_local(coverage_summary_tbl, storage_tbl, doa_summary_tbl, column_spacing_deg, half_spacing_deg)
    rows = {};
    rows = add_keypoint_row_local(rows, 'column_spacing_deg', column_spacing_deg, 'actual adjacent column spacing from arr_cyl XY coordinates');
    rows = add_keypoint_row_local(rows, 'half_column_spacing_deg', half_spacing_deg, 'single-target nearest-column half spacing');
    rows = add_keypoint_row_local(rows, 'coverage_R06_pair_center', coverage_value_local(coverage_summary_tbl, "pair_center", 0.6), 'Part A pair-center coverage at R=0.6');
    rows = add_keypoint_row_local(rows, 'coverage_R10_pair_center', coverage_value_local(coverage_summary_tbl, "pair_center", 1.0), 'Part A pair-center coverage at R=1.0');
    rows = add_keypoint_row_local(rows, 'coverage_R15_pair_center', coverage_value_local(coverage_summary_tbl, "pair_center", 1.5), 'Part A pair-center coverage at R=1.5');
    rows = add_keypoint_row_local(rows, 'coverage_R20_pair_center', coverage_value_local(coverage_summary_tbl, "pair_center", 2.0), 'Part A pair-center coverage at R=2.0');
    rows = add_keypoint_row_local(rows, 'coverage_R06_strong_center', coverage_value_local(coverage_summary_tbl, "strong_target", 0.6), 'Part A strong-target coverage at R=0.6');
    rows = add_keypoint_row_local(rows, 'coverage_R10_strong_center', coverage_value_local(coverage_summary_tbl, "strong_target", 1.0), 'Part A strong-target coverage at R=1.0');
    rows = add_keypoint_row_local(rows, 'coverage_R15_strong_center', coverage_value_local(coverage_summary_tbl, "strong_target", 1.5), 'Part A strong-target coverage at R=1.5');
    rows = add_keypoint_row_local(rows, 'coverage_R20_strong_center', coverage_value_local(coverage_summary_tbl, "strong_target", 2.0), 'Part A strong-target coverage at R=2.0');
    rows = add_keypoint_row_local(rows, 'recommended_template_R_deg', 2.0, 'recommended canonical template coverage for current d_max<=0.8 DOA validation');
    rows = add_keypoint_row_local(rows, 'recommended_runtime_default_R_deg', 1.5, 'recommended MATLAB/runtime default local sweep');
    rows = add_keypoint_row_local(rows, 'recommended_runtime_expand_R_deg', 2.0, 'recommended low-confidence adaptive expansion sweep');
    rows = add_keypoint_row_local(rows, 'template_size_MB_int16_R20', storage_value_local(storage_tbl, "2d_music_center_subarray", 2.0, 'template_size_MB_int16'), 'type-1 int16 complex template size at R=2.0');
    rows = add_keypoint_row_local(rows, 'compute_ratio_R20_vs_R06', storage_value_local(storage_tbl, "2d_music_center_subarray", 2.0, 'compute_ratio_vs_R06'), '2D MUSIC grid ratio R=2.0 vs R=0.6');
    rows = add_keypoint_row_local(rows, 'R06_success_overall', doa_overall_value_local(doa_summary_tbl, 0.6, 'success_rate'), 'Part C overall lazy success at R=0.6');
    rows = add_keypoint_row_local(rows, 'R10_success_overall', doa_overall_value_local(doa_summary_tbl, 1.0, 'success_rate'), 'Part C overall lazy success at R=1.0');
    rows = add_keypoint_row_local(rows, 'R15_success_overall', doa_overall_value_local(doa_summary_tbl, 1.5, 'success_rate'), 'Part C overall lazy success at R=1.5');
    rows = add_keypoint_row_local(rows, 'R20_success_overall', doa_overall_value_local(doa_summary_tbl, 2.0, 'success_rate'), 'Part C overall lazy success at R=2.0');
    rows = add_keypoint_row_local(rows, 'R06_false_high', doa_overall_value_local(doa_summary_tbl, 0.6, 'false_high_confidence_rate'), 'Part C false-high at R=0.6');
    rows = add_keypoint_row_local(rows, 'R20_false_high', doa_overall_value_local(doa_summary_tbl, 2.0, 'false_high_confidence_rate'), 'Part C false-high at R=2.0');
    rows = add_keypoint_row_local(rows, 'weak_target_boundary_preserved_flag', weak_boundary_preserved_flag_local(doa_summary_tbl), 'weak beta=0.3 remains protected by low confidence/boundary behavior');
    rows = add_keypoint_row_local(rows, 'large_el_branch_preserved_flag', large_el_preserved_flag_local(doa_summary_tbl), 'large-el reference branch remains successful and safe');
    keypoints = cell2table(vertcat(rows{:}), 'VariableNames', {'keypoint', 'value', 'note'});
end

function val = coverage_value_local(summary_tbl, mode, R)
    mask = string(summary_tbl.center_mode) == string(mode) & abs(summary_tbl.R - R) < 1e-9;
    if any(mask)
        val = summary_tbl.coverage_rate_both(find(mask, 1));
    else
        val = NaN;
    end
end

function val = storage_value_local(tbl, template_type, R, var_name)
    mask = string(tbl.template_type) == string(template_type) & abs(tbl.R - R) < 1e-9;
    if any(mask)
        val = tbl.(var_name)(find(mask, 1));
    else
        val = NaN;
    end
end

function val = doa_overall_value_local(tbl, R, var_name)
    mask = string(tbl.summary_level) == "R_overall" & abs(tbl.R - R) < 1e-9;
    if any(mask)
        val = tbl.(var_name)(find(mask, 1));
    else
        val = NaN;
...
function flag = weak_boundary_preserved_flag_local(tbl)
    mask = string(tbl.summary_level) == "scene_R" & string(tbl.scene) == "weak_beta_boundary";
    if ~any(mask)
        flag = 0;
        return
    end
    safe = all(tbl.false_high_confidence_rate(mask) <= 0.05 & tbl.boundary_missed_rate(mask) <= 0.05);
    still_boundary = any(tbl.low_confidence_rate(mask) > 0.05 | tbl.success_rate(mask) < 0.8);
    flag = double(safe && still_boundary);
end

function flag = large_el_preserved_flag_local(tbl)
    mask = string(tbl.summary_level) == "scene_R" & string(tbl.scene) == "large_el_reference";
    if ~any(mask)
        flag = 0;
        return
    end
    flag = double(all(tbl.success_rate(mask) >= 0.8 & tbl.false_high_confidence_rate(mask) <= 0.05));
end

function plot_coverage_vs_R_by_center_mode_local(tbl, path_out)
    fig = figure('Visible', 'off');
    modes = unique(string(tbl.center_mode), 'stable');
    hold on
    for im = 1:numel(modes)
        R_vals = unique(tbl.R(string(tbl.center_mode) == modes(im)), 'stable');
        y = nan(size(R_vals));
        for ir = 1:numel(R_vals)
            mask = string(tbl.center_mode) == modes(im) & tbl.R == R_vals(ir);
            y(ir) = mean(tbl.both_inside_range(mask));
        end
        plot(R_vals, y, '-o', 'LineWidth', 1.5);
    end
    grid on; ylim([0 1.05]); xlabel('R_\theta (deg)'); ylabel('both-target coverage');
    title('Dynamic center coverage vs R');
    legend(cellstr(modes), 'Location', 'best');
    saveas(fig, path_out); close(fig);
end

function plot_required_R_vs_pair_sep_local(tbl, path_out)
    fig = figure('Visible', 'off');
    modes = unique(string(tbl.center_mode), 'stable');
    hold on
    for im = 1:numel(modes)
        mode_mask = string(tbl.center_mode) == modes(im);
        sep_vals = unique(tbl.pair_sep(mode_mask), 'stable');
        y = nan(size(sep_vals));
        for is = 1:numel(sep_vals)
            mask = mode_mask & abs(tbl.pair_sep - sep_vals(is)) < 1e-9;
            y(is) = max(tbl.required_R(mask));
        end
        plot(sep_vals, y, '-o', 'LineWidth', 1.5);
    end
    grid on; xlabel('pair separation d (deg)'); ylabel('worst required R_\theta (deg)');
    title('Worst required R by pair separation');
    legend(cellstr(modes), 'Location', 'best');
    saveas(fig, path_out); close(fig);
end

function plot_delta_theta_distribution_by_mode_local(tbl, path_out)
    fig = figure('Visible', 'off');
    modes = unique(string(tbl.center_mode), 'stable');
    tiledlayout(numel(modes), 1, 'TileSpacing', 'compact');
    for im = 1:numel(modes)
        nexttile
        mask = string(tbl.center_mode) == modes(im) & tbl.R == max(tbl.R);
        vals = [tbl.delta_theta1(mask); tbl.delta_theta2(mask)];
        histogram(vals, 24);
        grid on; xlabel('\Delta\theta target (deg)'); ylabel('count');
        title(char(modes(im)));
    end
    saveas(fig, path_out); close(fig);
end

function plot_template_size_vs_R_local(tbl, path_out)
    fig = figure('Visible', 'off');
    types = unique(string(tbl.template_type), 'stable');
    hold on
    for it = 1:numel(types)
        mask = string(tbl.template_type) == types(it);
        plot(tbl.R(mask), tbl.template_size_MB_int16(mask), '-o', 'LineWidth', 1.5);
    end
    grid on; xlabel('R_\theta (deg)'); ylabel('int16 complex size (MB)');
    title('Template storage vs R');
    legend(cellstr(types), 'Location', 'best');
    saveas(fig, path_out); close(fig);
end

function plot_compute_ratio_vs_R_local(tbl, path_out)
    fig = figure('Visible', 'off');
    base = tbl(string(tbl.template_type) == "2d_music_center_subarray", :);
    plot(base.R, base.compute_ratio_vs_R06, '-o', 'LineWidth', 1.5);
    grid on; xlabel('R_\theta (deg)'); ylabel('grid compute ratio vs R=0.6');
    title('2D MUSIC grid ratio vs R');
    saveas(fig, path_out); close(fig);
end

function plot_doa_success_vs_R_local(tbl, path_out)
    fig = figure('Visible', 'off');
    T = tbl(string(tbl.summary_level) == "scene_R", :);
    scenes = unique(string(T.scene), 'stable');
    hold on
    for is = 1:numel(scenes)
        mask = string(T.scene) == scenes(is);
        plot(T.R(mask), T.success_rate(mask), '-o', 'LineWidth', 1.5);
    end
    grid on; ylim([0 1.05]); xlabel('R_\theta (deg)'); ylabel('success rate');
    title('DOA lazy success vs R');
    legend(cellstr(scenes), 'Location', 'best');
    saveas(fig, path_out); close(fig);
end

function plot_doa_low_confidence_vs_R_local(tbl, path_out)
    fig = figure('Visible', 'off');
    T = tbl(string(tbl.summary_level) == "scene_R", :);
    scenes = unique(string(T.scene), 'stable');
    hold on
    for is = 1:numel(scenes)
        mask = string(T.scene) == scenes(is);
        plot(T.R(mask), T.low_confidence_rate(mask), '-o', 'LineWidth', 1.5);
    end
    grid on; ylim([0 1.05]); xlabel('R_\theta (deg)'); ylabel('low-confidence rate');
    title('Low confidence vs R');
    legend(cellstr(scenes), 'Location', 'best');
    saveas(fig, path_out); close(fig);
end

function plot_doa_runtime_vs_R_local(tbl, path_out)
    fig = figure('Visible', 'off');
    T = tbl(string(tbl.summary_level) == "R_overall", :);
    yyaxis left
    plot(T.R, T.mean_runtime_sec, '-o', 'LineWidth', 1.5);
    ylabel('mean runtime (s)');
    yyaxis right
    plot(T.R, T.p90_runtime_sec, '-s', 'LineWidth', 1.5);
    ylabel('p90 runtime (s)');
    grid on; xlabel('R_\theta (deg)'); title('Runtime vs R');
    saveas(fig, path_out); close(fig);
end

function write_dynamic_center_range_record_doc_local(record_doc_path, result_dir, keypoints_tbl, ...
    coverage_summary_tbl, storage_tbl, doa_summary_tbl, column_spacing_deg, half_spacing_deg, ...
    theta_sep_ref, Metkl, quick_mode, elapsed_doa_sec)
    fid = fopen(record_doc_path, 'w');
    cleanup = onCleanup(@() safe_fclose_local(fid));
    fprintf(fid, '# 第8.7步 9 双目标动态选列与Delta搜索范围验证记录\n\n');
    fprintf(fid, '## 范围\n\n');
    fprintf(fid, '- 脚本：`space_smooth_music_B_cylindrical_level3_dynamic_center_range_validation.m`\n');
    fprintf(fid, '- 短名 runner：`space_smooth_music_B_cylindrical_level3_dyn_range.m`\n');
    fprintf(fid, '- 结果目录：`%s/`\n', string(get_last_path_name_dynamic_local(result_dir)));
    fprintf(fid, '- 本轮不做 FPGA、不做定点量化、不做 MC=100、不改第 7B 分流阈值或算法逻辑。\n');
    fprintf(fid, '- Part C 复用第 7B lazy cascade local functions，仅改变局部方位搜索范围 R。\n');
    fprintf(fid, '- Monte Carlo：Metkl=%d，quick_mode=%d，T_snap=260，SNR=[8,16]。\n\n', Metkl, quick_mode);

    fprintf(fid, '## 理论背景\n\n');
    fprintf(fid, '### 1. 单目标动态选列\n\n');
    fprintf(fid, '设目标真实方位为 theta_target，阵元列方位集合为 phi_q，动态选列中心为最近列：\n\n');
    fprintf(fid, '$$theta_c = nearest\\_column(theta\\_target)$$\n\n');
    fprintf(fid, '则局部偏移为：\n\n');
    fprintf(fid, '$$Delta\\_theta = theta\\_target - theta_c$$\n\n');
    fprintf(fid, '若列间隔为 Delta_phi_col，则：\n\n');
    fprintf(fid, '$$|Delta\\_theta| <= Delta\\_phi\\_col / 2$$\n\n');
    fprintf(fid, '本轮从 `arr_cyl` 生成的实际 XY 坐标反算列角，得到 Delta_phi_col = %.9f deg，因此半列间隔为 %.9f deg。这个界只是单目标或目标中心的最近列吸附误差，不等于双目标场景中每个目标的最大偏移。\n\n', column_spacing_deg, half_spacing_deg);

    fprintf(fid, '### 2. 双目标按融合中心选列\n\n');
    fprintf(fid, '设 theta1 = theta0 - d/2，theta2 = theta0 + d/2。如果粗检测只有一个融合峰并接近 theta0，则 theta_c = nearest_column(theta0)。定义 delta = theta0 - theta_c，则 |delta| <= %.9f deg。\n\n', half_spacing_deg);
    fprintf(fid, '两个目标相对中心列的偏移为：\n\n');
    fprintf(fid, '$$Delta\\_theta1 = delta - d/2$$\n\n');
    fprintf(fid, '$$Delta\\_theta2 = delta + d/2$$\n\n');
    fprintf(fid, '因此局部搜索半宽至少需要满足：\n\n');
    fprintf(fid, '$$R\\_theta >= %.9f\\ deg + d\\_max/2 + epsilon\\_coarse + epsilon\\_margin$$\n\n', half_spacing_deg);

    fprintf(fid, '### 3. 双目标按强目标选列\n\n');
    fprintf(fid, '如果幅度差较大，粗峰可能被强目标主导。若 theta_c = nearest_column(theta_strong)，强目标偏移满足 |Delta_theta_strong| <= %.9f deg。若弱目标与强目标间隔为 d，则弱目标偏移可能达到：\n\n', half_spacing_deg);
    fprintf(fid, '$$|Delta\\_theta\\_weak| <= %.9f\\ deg + d$$\n\n', half_spacing_deg);
    fprintf(fid, '因此局部搜索半宽需要满足：\n\n');
    fprintf(fid, '$$R\\_theta >= %.9f\\ deg + d\\_max + epsilon\\_coarse + epsilon\\_margin$$\n\n', half_spacing_deg);

    fprintf(fid, '### 4. 观测粗峰选列\n\n');
    fprintf(fid, '工程中也可能先用低成本粗谱或波束扫描得到 theta_coarse = argmax P_coarse(theta)，再选 theta_c = nearest_column(theta_coarse)。theta_coarse 会随 beta、phase、rho、SNR 和 pair separation 改变。本轮 Part A 对 observed coarse peak 使用解析代理：\n\n');
    fprintf(fid, '$$theta\\_eff = (theta1 + beta\\ theta2)/(1+beta)$$\n\n');
    fprintf(fid, '并把 theta_eff 吸附到最近实际列；本轮未实现额外 beamformer simulated coarse peak，以避免把搜索范围验证扩展成新的粗测角算法实验。\n\n');

    fprintf(fid, '### 5. 本轮核心问题\n\n');
    fprintf(fid, '本轮验证 ±0.6 deg 是否足够、±1.0 deg 是否仅覆盖单目标吸附、±1.5 deg 是否覆盖 pair-center 双目标、±2.0 deg 是否覆盖 strong-target/粗中心偏置，并观察扩大 R 后 success、false-high、boundary-missed、runtime 和模板量的变化。\n\n');

    fprintf(fid, '## 几何覆盖结果\n\n');
    fprintf(fid, '- sep_factor=10 对应的实际 theta_sep = %.9f deg，已替代名义 0.264 deg。\n\n', theta_sep_ref);
    fprintf(fid, '| center_mode | R | coverage_both | worst_required_R | recommended_min_R |\n');
    fprintf(fid, '|---|---:|---:|---:|---:|\n');
    for i = 1:height(coverage_summary_tbl)
        fprintf(fid, '| %s | %.1f | %.3f | %.6f | %.1f |\n', ...
            coverage_summary_tbl.center_mode(i), coverage_summary_tbl.R(i), ...
            coverage_summary_tbl.coverage_rate_both(i), coverage_summary_tbl.worst_required_R_at_R(i), ...
            coverage_summary_tbl.recommended_min_R_for_mode(i));
    end
    ...
    fprintf(fid, '## 模板存储与计算量\n\n');
    fprintf(fid, '| template_type | R | N_delta | N_grid | ratio_vs_R06 | int16_MB |\n');
    fprintf(fid, '|---|---:|---:|---:|---:|---:|\n');
    for i = 1:height(storage_tbl)
        fprintf(fid, '| %s | %.1f | %d | %d | %.3f | %.3f |\n', ...
            storage_tbl.template_type(i), storage_tbl.R(i), storage_tbl.N_delta(i), ...
            storage_tbl.N_grid(i), storage_tbl.compute_ratio_vs_R06(i), storage_tbl.template_size_MB_int16(i));
    end
    fprintf(fid, '\n');

    fprintf(fid, '## DOA route 摘要\n\n');
    T = doa_summary_tbl(string(doa_summary_tbl.summary_level) == "R_overall", :);
    fprintf(fid, '| R | success | false_high | boundary_missed | low_confidence | boundary_unreliable | mean_runtime | p90_runtime | 2D exec | pair-local exec |\n');
    fprintf(fid, '|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n');
    for i = 1:height(T)
        fprintf(fid, '| %.1f | %.3f | %.3f | %.3f | %.3f | %.3f | %.4f | %.4f | %.3f | %.3f |\n', ...
            T.R(i), T.success_rate(i), T.false_high_confidence_rate(i), T.boundary_missed_rate(i), ...
            T.low_confidence_rate(i), T.boundary_unreliable_rate(i), T.mean_runtime_sec(i), ...
            T.p90_runtime_sec(i), T.execution_rate_2dmusic(i), T.execution_rate_pair_local(i));
    end
    fprintf(fid, '\n');
    ...
    fprintf(fid, '## 判断\n\n');
    fprintf(fid, '- R=0.6 只适合理想 azCtr 已经很接近真实中心的仿真，不足以覆盖动态最近列选中心的双目标最坏情况。\n');
    fprintf(fid, '- R=1.0 基本对应单目标最近列吸附量级，对双目标 pair-center 已不充分。\n');
    fprintf(fid, '- R=1.5 对本轮 DOA 中 d<=0.8 deg 的等幅/融合中心双目标是较合理 runtime 默认范围；若几何 d_max 扩展到 1.2 deg，理论上还需要更宽余量。\n');
    fprintf(fid, '- R=2.0 对本轮 strong-target d<=0.8 deg 场景更稳，可作为 canonical template 覆盖和低置信扩展范围。\n');
    fprintf(fid, '- beta=0.3 弱目标仍属于 weak-target boundary；扩大搜索范围不能解决弱目标可辨识性，只能避免目标落出局部网格。\n');
    fprintf(fid, '- large-el reference 用于确认扩大 R 没有破坏大俯仰差分支。\n\n');

    fprintf(fid, '## 推荐策略\n\n');
    fprintf(fid, '- FPGA canonical template Delta_theta 覆盖范围：R_template = %.1f deg。\n', keypoint_value_local(keypoints_tbl, 'recommended_template_R_deg'));
    fprintf(fid, '- MATLAB/runtime 默认搜索范围：R_runtime_default = %.1f deg。\n', keypoint_value_local(keypoints_tbl, 'recommended_runtime_default_R_deg'));
    fprintf(fid, '- runtime adaptive expansion：低置信或边界迹象时扩展到 R = %.1f deg。\n', keypoint_value_local(keypoints_tbl, 'recommended_runtime_expand_R_deg'));
    fprintf(fid, '- 当前 ±0.6 deg 只保留为理想局部仿真范围，不建议作为工程动态选列默认范围。\n');
    fprintf(fid, '- 本轮完成后可以进入定点量化验证，但需要在定点报告中保留 R_template 与 runtime ROI/扩展策略的区分。\n\n');
    fprintf(fid, '## 输出\n\n');
    fprintf(fid, '- Result dir: `%s`\n', result_dir);
    fprintf(fid, '- elapsed_doa_sec=%.2f\n', elapsed_doa_sec);
end

function name = get_last_path_name_dynamic_local(path_in)
