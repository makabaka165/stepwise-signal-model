function [Y, truth] = make_cyl_el_separation_snapshots(x, y, z, az_pair_deg, el_pair_deg, lambda, L, snr_db, source_mode, varargin)
%MAKE_CYL_EL_SEPARATION_SNAPSHOTS Generate cylindrical snapshots with separate elevations.

if nargin < 9
    error('make_cyl_el_separation_snapshots:NotEnoughInputs', 'Nine inputs are required.');
end
...
if ~(isscalar(L) && isfinite(L) && L > 0 && L == floor(L))
    error('make_cyl_el_separation_snapshots:InvalidL', 'L must be a positive integer.');
end
if ~(isscalar(snr_db) && isfinite(snr_db))
    error('make_cyl_el_separation_snapshots:InvalidSnr', 'snr_db must be a finite scalar.');
end
...
switch source_mode
    ...
    otherwise
        error('make_cyl_el_separation_snapshots:UnknownSourceMode', 'Unknown source_mode: %s', source_mode);
end
...
truth.el_center_deg = mean(el_pair_deg);
truth.el_sep_deg = abs(diff(el_pair_deg));
...
if mod(numel(varargin), 2) ~= 0
    error('make_cyl_el_separation_snapshots:InvalidNameValue', 'Name-value options must be paired.');
end
for idx = 1:2:numel(varargin)
    ...
    switch name
        ...
        case 'seed'
            opts.seed = value;
        otherwise
            error('make_cyl_el_separation_snapshots:UnknownOption', 'Unknown option: %s', name);
    end
end
if ~(isscalar(opts.beta) && isfinite(opts.beta) && opts.beta > 0)
    error('make_cyl_el_separation_snapshots:InvalidBeta', 'AmplitudeRatio must be a positive finite scalar.');
end
if ~(isscalar(opts.coherent_phase_deg) && isfinite(opts.coherent_phase_deg))
    error('make_cyl_el_separation_snapshots:InvalidCoherentPhase', 'CoherentPhaseDeg must be a finite scalar.');
end
if ~isempty(opts.seed) && ~(isscalar(opts.seed) && isfinite(opts.seed))
    error('make_cyl_el_separation_snapshots:InvalidSeed', 'Seed must be empty or a finite scalar.');
end
end
