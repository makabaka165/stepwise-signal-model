% Step 8.8B Y_work Doppler de-rotation interface validation.
%
% Scope:
% 1) Reuse the Step 8.8 frontend and Step 8.7 shared-center lazy cascade.
% 2) Compare no / minus / plus Doppler de-rotation on the same observation.
% 3) Validate interface behavior only; do not change Step 8.7 route gates.
%
...
result_dir = fullfile(script_dir, 'results_step8_8b_ywork_derotation_validation');
if ~exist(result_dir, 'dir')
    mkdir(result_dir);
end

log_path = fullfile(result_dir, 'step8_8b_ywork_derotation_validation.log');
fid_log = fopen(log_path, 'w');
cleanup_log = onCleanup(@() safe_fclose_local(fid_log));

summary_csv_path = fullfile(result_dir, 'step8_8b_ywork_derotation_summary.csv');
keypoints_csv_path = fullfile(result_dir, 'step8_8b_ywork_derotation_keypoints.csv');
trial_csv_path = fullfile(result_dir, 'step8_8b_ywork_derotation_trial.csv');
mat_path = fullfile(result_dir, 'step8_8b_ywork_derotation_result.mat');
record_doc_path = fullfile(script_dir, '第8.8B步_Ywork多普勒补偿接口验证记录.md');

quick_mode = strcmpi(getenv('STEP88B_QUICK_MODE'), '1');
if quick_mode
    Metkl = 5;
else
    Metkl = 30;
...
total_mode_trials = total_base_trials * numel(modes);

log_msg_local(fid_log, 'Step 8.8B Y_work Doppler de-rotation interface validation');
...
if quick_mode
    log_msg_local(fid_log, 'WARNING: quick_mode=1. This run is quick trend only and not formal conclusion.');
end
log_msg_local(fid_log, 'Modes: %s', strjoin(cellstr(modes), ', '));
log_msg_local(fid_log, 'Scope: same frontend_out and selected 65-column work array, only Y_work slow-time phase changes.');
...
log_msg_local(fid_log, 'Original Step 8.8 result directories are not overwritten.');
...
log_msg_local(fid_log, 'Doppler axis: fdAxis uses fftshift order, vAxis=-fdAxis*lambda/2, fd_true=%.6f Hz.', cfg88.fd_true);
...
for isc = 1:numel(scenarios)
    ...
    for imc = 1:Metkl
        ...
        mode_rows = cell(numel(modes), 1);
        for imode = 1:numel(modes)
            mode_name = modes(imode);
            if cfar_ok && frontend_out.selected_center_valid_flag
                enhance_in = frontend_to_step87_input_derot_local(frontend_out, pc_like, cfg88, array_geom, mode_name);
            else
                enhance_in = make_empty_enhance_input_local(frontend_out, cfg88);
                enhance_in.derotation_mode = mode_name;
                ...
                enhance_in.derotation_sign_used = derotation_sign_from_mode_local(mode_name);
            end
            ...
            ydiag = compute_ywork_diagnostics_local(enhance_in.Y_work);
            mode_rows{imode} = make_derot_trial_row_local(sc, imc, mode_name, frontend_out, enhance_in, ...
                enhance_out, timing, ydiag, cfg88);
        end
        mode_rows = add_mode_comparisons_local(mode_rows);
        for imode = 1:numel(modes)
            row_idx = row_idx + 1;
...
summary_tbl = build_step88b_summary_table_local(trial_tbl, modes);
keypoints_tbl = build_step88b_keypoints_local(trial_tbl, summary_tbl, Metkl, quick_mode, cfg88);
...
plot_success_no_vs_derotation_local(summary_tbl, fullfile(result_dir, 'success_no_vs_derotation.png'));
plot_agreement_metric_local(summary_tbl, 'route', fullfile(result_dir, 'route_agreement_derotation.png'));
plot_agreement_metric_local(summary_tbl, 'confidence', fullfile(result_dir, 'confidence_agreement_derotation.png'));
plot_az_el_diff_between_modes_local(summary_tbl, fullfile(result_dir, 'az_el_diff_between_modes.png'));
plot_eigen_ratio_no_vs_derotation_local(summary_tbl, fullfile(result_dir, 'eigen_ratio_no_vs_derotation.png'));
plot_runtime_no_vs_derotation_local(summary_tbl, fullfile(result_dir, 'runtime_no_vs_derotation.png'));

save(mat_path, 'trial_tbl', 'summary_tbl', 'keypoints_tbl', 'cfg88', 'array_geom', ...
    'pc_model', 'elapsed_sec', 'Metkl', 'quick_mode', 'modes', '-v7.3');

write_step88b_record_doc_local(record_doc_path, keypoints_tbl, summary_tbl, result_dir, elapsed_sec, Metkl, quick_mode, cfg88);
