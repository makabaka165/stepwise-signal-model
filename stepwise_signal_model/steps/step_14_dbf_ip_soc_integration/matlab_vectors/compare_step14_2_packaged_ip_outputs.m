function compare_step14_2_packaged_ip_outputs()
%COMPARE_STEP14_2_PACKAGED_IP_OUTPUTS Compare packaged IP XSim output to Step14.1 golden CSV.
...
packagedIpTbPassFlag = isTrueMetric(tbSummary, 'packaged_ip_tb_pass_flag');

packagedIpOutputMatchFlag = expectedRows == 14 && actualRows == 14 && matchedRows == 14 && ...
    missingCount == 0 && duplicateCount == 0 && tdataMismatchCount == 0 && ...
    zValueMismatchCount == 0 && flagMismatchCount == 0 && tlastMismatchCount == 0 && ...
    beamOrderMismatchCount == 0;
packagedIpComparePassFlag = packagedIpOutputMatchFlag && packagedIpTbPassFlag;
comparisonStatus = ternary(packagedIpComparePassFlag, 'pass', 'fail');
...
summaryPairs = {
    ...
    'packaged_ip_output_match_flag', boolStr(packagedIpOutputMatchFlag);
    'packaged_ip_tb_pass_flag', boolStr(packagedIpTbPassFlag);
    'packaged_ip_compare_pass_flag', boolStr(packagedIpComparePassFlag);
    'formal_result_claimed', 'false';
};
writeKeyValueCsv(fullfile(cmpDir, 'step14_2_packaged_ip_compare_summary.csv'), summaryPairs);

fprintf('Step14.2 packaged IP compare status: %s\n', comparisonStatus);

end

function mustExist(filePath)
if ~exist(filePath, 'file')
    error('Step14.2:MissingInput', 'Required file is missing: %s', filePath);
end
end
...
for k = 1:numel(cols)
    if ~any(strcmp(names, cols{k}))
        error('Step14.2:MissingColumn', 'Missing column %s in %s.', cols{k}, filePath);
    end
end
end
