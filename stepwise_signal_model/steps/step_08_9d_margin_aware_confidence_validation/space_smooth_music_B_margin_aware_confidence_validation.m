...
result_dir = fullfile(script_dir, 'results_step8_9d_margin_aware_confidence_validation');
if ~exist(result_dir, 'dir')
    mkdir(result_dir);
end

log_path = fullfile(result_dir, 'step8_9d_margin_aware_confidence_validation.log');
fid_log = fopen(log_path, 'w');
cleanup_log = onCleanup(@() safe_fclose_local(fid_log));

trial_diag_csv_path = fullfile(result_dir, 'step8_9d_trial_diagnostics.csv');
guard_summary_csv_path = fullfile(result_dir, 'step8_9d_guard_sweep_summary.csv');
worst_replay_csv_path = fullfile(result_dir, 'step8_9d_worst_case_replay.csv');
keypoints_csv_path = fullfile(result_dir, 'step8_9d_guard_policy_keypoints.csv');
recommendations_csv_path = fullfile(result_dir, 'step8_9d_recommendations.csv');
mat_path = fullfile(result_dir, 'step8_9d_result.mat');
record_doc_path = fullfile(script_dir, '第8.9D步_margin-aware置信度与candidate-tie-guard验证记录.md');

required_files = {input_step89c_trial_csv, input_step89c_summary_csv, input_step89c_worst_csv, ...
...
for i = 1:numel(required_files)
    if ~exist(required_files{i}, 'file')
        error('Missing required Step 8.9D input: %s', required_files{i});
    end
end
...
replay_mode_names = ["double_baseline", "y16_coeff24_all", "y32float_coeff24", ...
    "y16_coeff32float", "float32_all", "y16_rank1_2d_coeff24_rest16"];
...
replication_note = "targeted replay uses the same Step 8.9C scenario order, base_seed, and MC seed rule; original Step 8.9C result directory is read-only input.";
...
target_tbl = select_step89d_targets_local(T89C, W89C, Metkl, numel(all_quant_modes));
total_targeted_trials = height(target_tbl);
total_mode_trials = total_targeted_trials * numel(quant_modes);

log_msg_local(fid_log, 'Step 8.9D margin-aware confidence / candidate tie guard validation');
log_msg_local(fid_log, 'Stage A: loaded Step 8.9C trial rows=%d, worst-case rows=%d.', height(T89C), height(W89C));
...
log_msg_local(fid_log, 'No edits to Step 8.7 / Step 8.8 / Step 8.9C original files or result directories.');
...
trial_diag_tbl = struct2table([trial_rows{:}]);
guard_policies = build_guard_policies_step89d_local();
guard_eval_tbl = evaluate_guard_policies_step89d_local(trial_diag_tbl, guard_policies);
guard_summary_tbl = build_guard_summary_step89d_local(guard_eval_tbl);
worst_replay_tbl = build_worst_replay_step89d_local(guard_eval_tbl, 80);
recommendations_tbl = build_recommendations_step89d_local(guard_summary_tbl);
keypoints_tbl = build_keypoints_step89d_local(guard_summary_tbl, target_tbl, exact_same_trial_replication_flag);

log_msg_local(fid_log, 'Targeted replay finished in %.2f sec.', elapsed_sec);
log_msg_local(fid_log, 'Best guard policy: %s', keypoint_note_from_table_local(keypoints_tbl, 'best_guard_policy'));
log_msg_local(fid_log, 'Writing CSV/MAT outputs.');
writetable(trial_diag_tbl, trial_diag_csv_path);
writetable(guard_summary_tbl, guard_summary_csv_path);
writetable(worst_replay_tbl, worst_replay_csv_path);
...
plot_worst_case_route_flip_step89d_local(guard_eval_tbl, fullfile(result_dir, 'worst_case_route_flip_before_after.png'));
plot_margin_distribution_step89d_local(trial_diag_tbl, fullfile(result_dir, 'margin_distribution_changed_vs_unchanged.png'));
plot_guard_metric_step89d_local(guard_summary_tbl, 'output_equivalent_after_guard', 'guard sweep output equivalent', fullfile(result_dir, 'guard_sweep_output_equivalent.png'));
plot_guard_metric_step89d_local(guard_summary_tbl, 'safe_equivalent_rate', 'guard sweep safe equivalent', fullfile(result_dir, 'guard_sweep_safe_equivalent.png'));
plot_guard_metric_step89d_local(guard_summary_tbl, 'success_loss_due_to_guard', 'guard sweep success tradeoff', fullfile(result_dir, 'guard_sweep_success_tradeoff.png'));
plot_guard_metric_step89d_local(guard_summary_tbl, 'low_confidence_increment', 'guard sweep low-confidence tradeoff', fullfile(result_dir, 'guard_sweep_low_confidence_tradeoff.png'));
plot_az_diff_before_after_step89d_local(guard_eval_tbl, keypoints_tbl, fullfile(result_dir, 'az_diff_before_after_guard.png'));
plot_route_flip_matrix_step89d_local(guard_eval_tbl, keypoints_tbl, fullfile(result_dir, 'route_flip_matrix_before_after.png'));
plot_margin_vs_az_step89d_local(trial_diag_tbl, fullfile(result_dir, 'candidate_margin_vs_az_diff.png'));
plot_recommended_guard_step89d_local(guard_summary_tbl, keypoints_tbl, fullfile(result_dir, 'recommended_guard_policy.png'));

save(mat_path, 'trial_diag_tbl', 'guard_eval_tbl', 'guard_summary_tbl', 'worst_replay_tbl', ...
    'keypoints_tbl', 'recommendations_tbl', 'target_tbl', 'S89C', 'K89C', ...
    'cfg88', 'array_geom', 'pc_model', 'elapsed_sec', 'Metkl', 'quant_modes', ...
    'guard_policies', 'exact_same_trial_replication_flag', 'targeted_replay_flag', 'full_rerun_flag', '-v7.3');

write_step89d_record_doc_local(record_doc_path, keypoints_tbl, guard_summary_tbl, ...
    recommendations_tbl, result_dir, elapsed_sec, total_targeted_trials, ...
    exact_same_trial_replication_flag, targeted_replay_flag, full_rerun_flag, quant_modes, guard_policies);
