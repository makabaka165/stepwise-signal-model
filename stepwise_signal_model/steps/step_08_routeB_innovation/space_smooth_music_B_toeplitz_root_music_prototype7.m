...
proto_diag_count = zeros(nsep, nsnr);

lambda2_pre_tp_sum = zeros(nsep, nsnr);
lambda2_post_tp_sum = zeros(nsep, nsnr);
tp_residual_sum = zeros(nsep, nsnr);
...
result_dir = fullfile(script_dir, 'results_step8_routeB_toeplitz_root_music_proto7');
if ~exist(result_dir, 'dir')
    mkdir(result_dir);
end

log_lines = {};
log_lines = append_log(log_lines, 'Step 08 Prototype 7: Toeplitz-FBSS + Root-MUSIC fair compare');
log_lines = append_log(log_lines, 'Routes: baseline Route B, proto4e, proto6 array, proto7 toeplitz array, proto7 toeplitz beamspace.');
...
for iSep = 1:nsep
    ...
    for iSNR = 1:nsnr
        ...
        for metkl_num = 1:Metkl
            ...
            [doa_proto4e, num_peaks_proto4e] = DOA_three_music_new_route_centerT( ...
            ...
            if all(isfinite(debug_proto7_beam.root_distance_unit))
                root_dist_beam_sum(iSep, iSNR) = root_dist_beam_sum(iSep, iSNR) + ...
                    mean(debug_proto7_beam.root_distance_unit);
            end
            proto_diag_count(iSep, iSNR) = proto_diag_count(iSep, iSNR) + 1;

            if isfinite(debug_proto7_array.lambda2_over_noise_before_tp)
                lambda2_pre_tp_sum(iSep, iSNR) = lambda2_pre_tp_sum(iSep, iSNR) + ...
                    debug_proto7_array.lambda2_over_noise_before_tp;
            end
            if isfinite(debug_proto7_array.lambda2_over_noise_after_tp)
                lambda2_post_tp_sum(iSep, iSNR) = lambda2_post_tp_sum(iSep, iSNR) + ...
                    debug_proto7_array.lambda2_over_noise_after_tp;
            end
            if isfinite(debug_proto7_array.toeplitz_residual_norm)
                tp_residual_sum(iSep, iSNR) = tp_residual_sum(iSep, iSNR) + ...
                    debug_proto7_array.toeplitz_residual_norm;
            end
            ...
            if metkl_num == 1
                ...
                sample.doa_proto4e = doa_proto4e;
                sample.doa_proto6_array = doa_proto6_array;
                sample.doa_proto7_array = doa_proto7_array;
                sample.doa_proto7_beam = doa_proto7_beam;
                sample.debug_proto6_array = debug_proto6_array;
                sample.debug_proto7_array = debug_proto7_array;
                sample.debug_proto7_beam = debug_proto7_beam;
...
mean_fft_center_error_deg = fft_center_error_sum ./ max(proto_diag_count, 1);
std_fft_center_error_deg = sqrt(max(fft_center_error_sumsq ./ max(proto_diag_count, 1) - mean_fft_center_error_deg.^2, 0));
mean_fwhm_fft_deg = fwhm_fft_sum ./ max(proto_diag_count, 1);
std_fwhm_fft_deg = sqrt(max(fwhm_fft_sumsq ./ max(proto_diag_count, 1) - mean_fwhm_fft_deg.^2, 0));
mean_raw_search_ratio_4e = raw_search_ratio_4e_sum ./ max(proto_diag_count, 1);
mean_piecewise_search_ratio_4e = piecewise_search_ratio_4e_sum ./ max(proto_diag_count, 1);
mean_piecewise_span_ratio_4e = piecewise_span_ratio_4e_sum ./ max(proto_diag_count, 1);
mean_theta_search_4e_deg = theta_search_4e_sum ./ max(proto_diag_count, 1);
mean_routeB_beam_span_4e_deg = routeB_beam_span_4e_sum ./ max(proto_diag_count, 1);
lambda2_over_noise_array = lambda2_over_noise_array_sum ./ max(proto_diag_count, 1);
root_dist_array_mean = root_dist_array_sum ./ max(proto_diag_count, 1);
root_dist_beam_mean = root_dist_beam_sum ./ max(proto_diag_count, 1);
...
for iSep = 1:nsep
    log_lines = append_log(log_lines, ...
        'sep_factor=%d | SNR90 abs01 -> baseline=%s, proto4e=%s, proto6-array=%s, proto7-array=%s, proto7-beam=%s', ...
    ...
    log_lines = append_log(log_lines, ...
        'sep_factor=%d | SNR90 rel025 -> baseline=%s, proto4e=%s, proto6-array=%s, proto7-array=%s, proto7-beam=%s', ...
...
log_path = fullfile(result_dir, 'step8_routeB_toeplitz_root_music_proto7.log');
...
summary_path = fullfile(result_dir, 'step8_routeB_toeplitz_root_music_proto7_summary.csv');
...
fprintf(fid, ['route_name,sep_factor,theta_sep_deg,theta_a_deg,theta_b_deg,snr_db,' ...
    ...
    'lambda2_over_noise_array,root_dist_array_mean,root_dist_beam_mean,' ...
    ...
    for iSep = 1:nsep
        for iSNR = 1:nsnr
            fprintf(fid, ['%s,%d,%.6f,%.6f,%.6f,%d,' ...
                ...
                lambda2_pre_tp_mean(iSep, iSNR), lambda2_post_tp_mean(iSep, iSNR), tp_residual_mean(iSep, iSNR));
        end
    end
end
fclose(fid);

keypoints_path = fullfile(result_dir, 'step8_routeB_toeplitz_root_music_proto7_keypoints.csv');
...
mat_path = fullfile(result_dir, 'step8_routeB_toeplitz_root_music_proto7_result.mat');
save(mat_path, ...
    ...
    'mean_theta_search_4e_deg', 'mean_routeB_beam_span_4e_deg', ...
    'lambda2_over_noise_array', 'root_dist_array_mean', 'root_dist_beam_mean', ...
    'lambda2_pre_tp_mean', 'lambda2_post_tp_mean', 'tp_residual_mean', 'debug_samples');
...
for iSep = 1:nsep
    ...
    grid on
    ylabel('tol success');
    title(sprintf('Prototype7 tol success, bw/%d', sep_factor_list(iSep)));
    legend(route_names, 'Location', 'best');
end
xlabel('SNR (dB)');
saveas(fig1, fullfile(result_dir, 'step8_routeB_toeplitz_root_music_proto7_tol_success.png'));
close(fig1)
...
for iSep = 1:nsep
    ...
    grid on
    ylabel('RMSE (deg)');
    title(sprintf('Prototype7 RMSE, bw/%d', sep_factor_list(iSep)));
    legend(route_names, 'Location', 'best');
end
xlabel('SNR (dB)');
saveas(fig2, fullfile(result_dir, 'step8_routeB_toeplitz_root_music_proto7_rmse.png'));
close(fig2)
...
for iSep = 1:nsep
    ...
    grid on
    ylabel('raw success');
    title(sprintf('Prototype7 raw success, bw/%d', sep_factor_list(iSep)));
    legend(route_names, 'Location', 'best');
end
xlabel('SNR (dB)');
saveas(fig3, fullfile(result_dir, 'step8_routeB_toeplitz_root_music_proto7_raw_success.png'));
close(fig3)
...
for iSep = 1:nsep
    ...
    grid on
    ylabel('mean peaks');
    title(sprintf('Prototype7 mean num peaks, bw/%d', sep_factor_list(iSep)));
    legend(route_names, 'Location', 'best');
end
xlabel('SNR (dB)');
saveas(fig4, fullfile(result_dir, 'step8_routeB_toeplitz_root_music_proto7_mean_num_peaks.png'));
close(fig4)

fig5 = figure('Visible', 'off');
for iSep = 1:nsep
    subplot(nsep, 1, iSep);
    plot(snr_list, lambda2_pre_tp_mean(iSep, :), '-o', 'LineWidth', 1.1);
    hold on
    plot(snr_list, lambda2_post_tp_mean(iSep, :), '-s', 'LineWidth', 1.1);
    plot(snr_list, lambda2_pre_tp_mean(iSep, :) + 1, '--', 'LineWidth', 1.0);
    hold off
    grid on
    ylabel('\lambda_2 / noise');
    title(sprintf('Prototype7 lambda2 Toeplitz diag, bw/%d', sep_factor_list(iSep)));
    legend({'pre-tp', 'post-tp', 'pre-tp+1 ref'}, 'Location', 'best');
end
xlabel('SNR (dB)');
saveas(fig5, fullfile(result_dir, 'step8_routeB_toeplitz_root_music_proto7_lambda2_tp_diag.png'));
close(fig5)

disp('Step 08 Prototype 7 finished.');
disp('route_names =');
disp(route_names);
disp('snr90_abs01 =');
disp(snr90);
