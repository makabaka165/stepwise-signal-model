...
route_names = {'baseline_true_centerT', 'fft_guided_centerT', 'fft_guided_centerT_adaptive_window'};
...
log_lines = {};
log_lines = append_log(log_lines, 'Step 08 Route B FFT-guided adaptive window prototype 4b');
log_lines = append_log(log_lines, 'Prototype scope: keep FFT coarse center fixed, adapt theta_search_B and routeB_beam_span from FFT peak FWHM.');
log_lines = append_log(log_lines, 'Route comparison: baseline_true_centerT vs fft_guided_centerT vs fft_guided_centerT_adaptive_window.');
...
log_lines = append_log(log_lines, 'FFT settings: Nfft_spatial=%d, FWHM-driven adaptive window with gamma_search=%.2f, gamma_span=%.2f.', ...
...
for iSep = 1:nsep
    ...
    for iSNR = 1:nsnr
        ...
        log_lines = append_log(log_lines, ...
            'sample metkl=1 | center_true=%.4f center_fft=%.4f err_fft=%.4f fwhm=%.4f search_fixed=%.4f search_adapt=%.4f span_fixed=%.4f span_adapt=%.4f fallback_fwhm=%d doa_true=%s doa_fft=%s doa_adapt=%s', ...
            sample.RecvbeamC_true, sample.RecvbeamC_fft_coarse, sample.fft_center_error_deg, ...
            sample.fwhm_fft_deg, sample.theta_search_B_fixed, sample.theta_search_B_adapt, ...
            sample.routeB_beam_span_fixed, sample.routeB_beam_span_adapt, sample.used_fallback_fwhm, ...
...
for iSep = 1:nsep
    log_lines = append_log(log_lines, ...
        'SNR90 summary | bw/%d baseline=%s fft-guided=%s adaptive-window=%s', ...
...
params.prototype_note = 'Prototype 4b: FFT-guided Route B with FWHM-driven adaptive local search window.';

result_dir = fullfile(script_dir, 'results_step8_routeB_fft_guided_adaptive_window_proto4b');
if ~exist(result_dir, 'dir')
    mkdir(result_dir);
end

log_path = fullfile(result_dir, 'step8_routeB_fft_guided_adaptive_window_proto4b.log');
...
mat_path = fullfile(result_dir, 'step8_routeB_fft_guided_adaptive_window_proto4b_result.mat');
save(mat_path, ...
    'params', ...
    'route_names', ...
    'sep_factor_list', ...
...
for iSep = 1:nsep
    ...
    grid on
    ylabel('tol success');
    title(sprintf('Prototype 4b tol success, bw/%d', sep_factor_list(iSep)));
    legend(route_names, 'Location', 'best');
end
xlabel('SNR (dB)');
saveas(fig1, fullfile(result_dir, 'step8_routeB_fft_guided_adaptive_window_proto4b_tol_success.png'));
close(fig1)
...
for iSep = 1:nsep
    ...
    grid on
    ylabel('RMSE (deg)');
    title(sprintf('Prototype 4b RMSE, bw/%d', sep_factor_list(iSep)));
    legend(route_names, 'Location', 'best');
end
xlabel('SNR (dB)');
saveas(fig2, fullfile(result_dir, 'step8_routeB_fft_guided_adaptive_window_proto4b_rmse.png'));
close(fig2)
...
for iSep = 1:nsep
    ...
    grid on
    ylabel('raw success');
    title(sprintf('Prototype 4b raw success, bw/%d', sep_factor_list(iSep)));
    legend(route_names, 'Location', 'best');
end
xlabel('SNR (dB)');
saveas(fig3, fullfile(result_dir, 'step8_routeB_fft_guided_adaptive_window_proto4b_raw_success.png'));
close(fig3)
...
for iSep = 1:nsep
    ...
    grid on
    ylabel('mean peaks');
    title(sprintf('Prototype 4b mean num peaks, bw/%d', sep_factor_list(iSep)));
    legend(route_names, 'Location', 'best');
end
xlabel('SNR (dB)');
saveas(fig4, fullfile(result_dir, 'step8_routeB_fft_guided_adaptive_window_proto4b_mean_num_peaks.png'));
close(fig4)

disp('Prototype 4b finished.');
disp('route_names =');
disp(route_names);
disp('snr90 =');
disp(snr90);
